...
// Channel plumbing (Sept 2026): the mix is requested from WhatsApp, from
// RCS (lib/musicLoversRcsFlow.js) or from demo.html, and the follow-ups
// go back on the channel the listener is on. `from` is the WABA number
// for WhatsApp and the shared RCS agent for RCS (same as the rest of
// the Music Lovers RCS journey).
function senderFor(channel) {
  return channel === 'rcs' ? { from: businessConfig.RCS_AGENT_ID, channel: 'rcs' } : { from: musicConfig.FROM_WHATSAPP, channel: 'whatsapp' };
}

// RCS text messages are capped lower than WhatsApp's 4 096 chars (Vonage
// rejects long bodies), so a 90-minute set sheet gets its tail trimmed
// with a pointer to the playlist.
const RCS_TEXT_MAX = 2900;

async function sendText(phone, text, channel = 'whatsapp') {
  let body = text;
  if (channel === 'rcs' && body.length > RCS_TEXT_MAX) {
    body = body.slice(0, RCS_TEXT_MAX - 60).replace(/\n[^\n]*$/, '') + '\n…(full set sheet in the playlist)';
  }
  return sendVonageMessage({ ...senderFor(channel), to: phone, message_type: 'text', text: body });
}
...
async function buildAndSendMix(phone, userName, options = {}) {
  ...
  const channel = options.channel === 'rcs' ? 'rcs' : 'whatsapp';
  ...
  try {
    ...
    logEvent('outbound', `Personal mix (${source}, ${channel}, ${vibeKey(options.vibe)}, ${durationMin} min): started for ${redactPhone(phone)}`, phone);
    ...
    await sendText(phone, buildTracklistText(name, plan, playlistUrl || `(playlist could not be created on ${fromHenry ? "Henry's" : 'your'} account — see the tracklist below)`, vibe, durationMin, source), channel);
    ...
    try {
      ...
      const caption = `The arc of your ${vibe.label.toLowerCase()} mix: tempo per track, coloured by phase (warm-up → build → peak → landing). Dot size is the track's energy.`;
      if (channel === 'rcs') {
        // RCS image messages carry no caption — send the picture, then
        // the one-liner as a text.
        await sendVonageMessage({ ...senderFor('rcs'), to: phone, message_type: 'image', image: { url: `${base}/music-lovers/mix-arc/${setId}.png` } });
        await sendText(phone, caption, 'rcs');
      } else {
        await sendVonageMessage({
          ...senderFor('whatsapp'),
          to: phone,
          message_type: 'image',
          image: { url: `${base}/music-lovers/mix-arc/${setId}.png`, caption },
        });
      }
