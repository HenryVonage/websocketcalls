...
async function sendGenrePrompt(phone, userName) {
  // Two messages, not one — confirmed live (Sept 2026, first real test):
  // Vonage error 1020, "card.suggestions cannot exceed 4 items for the
  // given channel" when all 8 genre chips lived on the card below. RCS
  // cards cap suggestions at 4; plain RCS text messages don't (rcsFlow.js's
  // own [FLAT_TYPE_LIST] already sends 9 chips under a text message
  // successfully) — so the card just introduces the demo with its hero
  // image and no chips of its own, and the genre chips go out as a
  // separate text message right after. This was the silent failure behind
  // "no further outbound messages after the initial inbound greeting": the
  // very first RCS send was being rejected outright, so nothing ever
  // reached the phone.
  ...
  // One reply chip per genre — 8 total, within a text message's much
  // higher suggestion-count budget (see comment above). Chip text is the
  // genre name itself, so a tap's reply.id lands directly in
  // matchAllGenres() below with zero extra mapping — a genre's own display
  // name always contains its own GENRE_SYNONYMS entry.
  await sendVonageMessage({
    from: FROM,
    to: phone,
    channel: 'rcs',
    message_type: 'text',
    text: 'Choose a genre:',
    suggestions: musicConfig.GENRES.map((genre) => ({ type: 'reply', text: genre, postback_data: genre })),
  });
}
