...
async function sendExploreMoreMenu(phone) {
  ...
  logEvent('outbound', `Sent RCS explore-more chip menu to ${redactPhone(phone)}`, phone);
}
...
async function sendAlternateSong(phone, userName, state) {
  ...
  if (!alt) {
    logEvent('outbound', `Music Lovers RCS: no alternate track available for ${redactPhone(phone)} — skipped negative-feedback resend`, phone);
    return;
  }
...
async function handleNegativeFeedback(phone, userName, state) {
  ...
  if (songsSent >= 2) {
    logEvent('inbound', `Music Lovers RCS: negative feedback from ${redactPhone(phone)} after the 2-song cap — no further resend`, phone);
    return;
  }
...
async function handleAwaitingConfirmation(phone, userName, state, text) {
  ...
  if (musicConfig.RINGTONE_CONFIRM_RE.test(text)) {
    ...
    logEvent('outbound', `Sent RCS ringtone clip to ${redactPhone(phone)}`, phone);
    return;
  }
  ...
  if (/lyrics/i.test(text) && artist && title) {
    ...
    logEvent('outbound', `Sent lyrics link to ${redactPhone(phone)} (RCS)`, phone);
    return;
  }
  ...
  if (/biography/i.test(text) && artist) {
    ...
    logEvent('outbound', `Sent artist bio link to ${redactPhone(phone)} (RCS)`, phone);
    return;
  }
  ...
  if (/concert/i.test(text) && artist) {
    ...
    if (details) {
      await sendConcertsCard(phone, artist, details, state.spotifyTrackId);
      setMusicLoversState(phone, { concertArtist: artist, concertDetails: details });
      logEvent('outbound', `Sent RCS concert card to ${redactPhone(phone)}`, phone);
    } else {
      ...
      logEvent('outbound', `Sent concerts link to ${redactPhone(phone)} (RCS, no full event details available)`, phone);
    }
    return;
  }
  ...
  if (/more songs/i.test(text) && artist) {
    ...
    logEvent('outbound', `Sent more-songs link to ${redactPhone(phone)} (RCS)`, phone);
    return;
  }
  ...
  if (/into that type of song/i.test(text)) {
    ...
    logEvent('outbound', `Sent positive-feedback acknowledgment to ${redactPhone(phone)} (RCS)`, phone);
    return;
  }
...
async function handleTrackCarouselReply(phone, userName, state, text) {
  ...
  // concertArtist/concertDetails explicitly cleared here (not just left to
  // whatever `...state` carries in) — Henry's Sept 2026 report: calling the
  // ElevenLabs agent in after picking a NEW song from the carousel had it
  // talk about a DIFFERENT artist from an earlier test on this same phone
  // number. Root cause: ...state spreads in this phone's entire prior
  // state first, including a concertArtist/concertDetails pair set by an
  // earlier journey's "Concerts" lookup — buildMusicLoversDynamicVariables
  // (voiceHandlers.js) prefers state.concertArtist over state.artist, so
  // that stale artist won the moment a caller rang in without asking about
  // concerts for THIS new song. Same fix already applied to sendAlternateSong
  // below (the negative-feedback repick path) — this closes the same gap
  // for the initial carousel pick.
  const updatedState = { ...state, stage: 'awaiting_confirmation', ...track, songsSent: 1, concertArtist: undefined, concertDetails: undefined };
...
async function processMusicLoversRcs(fields) {
  ...
  try {
    ...
    logEvent('inbound', `Music Lovers RCS message from ${redactPhone(phone)}: "${fields.messageText}"`, phone);
    ...
    if (isResetGreetingFor(DEMOS.MUSIC_LOVERS, fields.messageText)) {
      ...
      logEvent('inbound', `Reset phrase detected — cleared Music Lovers state for ${redactPhone(phone)} (RCS)`, phone);
    }

    const state = getMusicLoversState(phone);

    // "Build me a 30 min workout mix" — same keyword handling as the
    // WhatsApp flow (parseMixRequest lives there), delivered back over
    // RCS (lib/musicLoversMix.js, channel: 'rcs'). Works at every stage.
    const { parseMixRequest } = require('./musicLoversFlow');
    const mixRequest = parseMixRequest(fields.messageText);
    if (mixRequest) {
      const { buildAndSendMix } = require('./musicLoversMix');
      logEvent('inbound', `Mix requested over RCS by ${redactPhone(phone)} (${mixRequest.vibe}, ${mixRequest.durationMin} min)`, phone);
      buildAndSendMix(phone, userName, { ...mixRequest, source: 'henry', channel: 'rcs' }).catch((err) => {
        console.error('buildAndSendMix (RCS keyword) failed:', err);
      });
      return;
    }
    ...
    if (!state.stage) {
      ...
      logEvent('outbound', `Sent RCS genre prompt to ${redactPhone(phone)}`, phone);
      return;
    }
    ...
    logEvent('outbound', `Music Lovers RCS handling failed for ${redactPhone(phone)}: ${err.message}`, phone);
    try {
      await sendVonageMessage({
        from: FROM,
        to: phone,
