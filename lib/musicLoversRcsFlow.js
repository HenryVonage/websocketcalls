...
async function sendGenrePrompt(phone, userName) {
  // One reply chip per genre — 8 total, within RCS's suggestion-count
  // budget (rcsFlow.js's [FLAT_TYPE_LIST] case already sends 9 under one
  // message). Chip text is the genre name itself, so a tap's reply.id
  // lands directly in matchAllGenres() below with zero extra mapping — a
  // genre's own display name always contains its own GENRE_SYNONYMS entry.
