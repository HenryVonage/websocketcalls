...
async function processTicketingWhatsapp(req, res) {
  ...
  try {
    ...
    // Henry's henry_form3 "Complete flow" button was tapped and the survey
    // completed live, but the expected nfm_reply branch below never fired
    // (no matching Render log line) — instead the inbound showed up as an
    // ordinary message_type "button" with button.text "Sent", which then
    // fell through to Claude/ticketingEngine and produced a raw, unhandled
    // "[T_WON]" marker (see the plain-text fallback guard below for why
    // that leaked to the customer as literal text). So Vonage's actual
    // webhook shape for a completed WhatsApp Flow on this account doesn't
    // match Meta's raw interactive.nfm_reply shape the way henry_form2's
    // (unverified in production) handling assumes. Logging the full raw
    // body for every inbound here — temporarily, until the real Flow
    // completion shape is confirmed from a live payload — so the next test
    // shows exactly what Vonage actually sends instead of guessing again.
