...
const { MARKERS, findProduct, downloadAsBase64 } = require('./ticketingMarkers');
...
// Reverse of W.PRODUCT_RETAILER_IDS (retailer id -> internal product id),
// used to translate a native WhatsApp cart/order inbound (see extractFields
// below) back into the same product ids the rest of this file already
// works with.
const RETAILER_ID_TO_PRODUCT_ID = Object.fromEntries(
  Object.entries(W.PRODUCT_RETAILER_IDS).map(([id, retailerId]) => [retailerId, id]),
);
...
function extractFields(body) {
  ...
  // Native WhatsApp cart checkout — visitor tapped "View items" on the
  // [T_PRODUCT_LIST] catalog message, added item(s) to WhatsApp's own cart
  // UI, then hit Send. That arrives as its own message_type: 'order'
  // inbound (confirmed live via Render logs), structurally unlike every
  // other reply this file handles — there's no free text or button id to
  // classify, just a list of catalog product items the visitor already
  // picked. Vonage's exact field name for the order payload hasn't been
  // double-checked against a second live payload yet, so this checks a
  // couple of plausible shapes; processTicketingWhatsapp additionally logs
  // the raw body.order for messageType 'order' so a mismatch here is easy
  // to spot and fix from Render logs alone.
  const orderProductItems = body.order?.product_items ?? body.order?.items ?? [];
  const orderedProductIds = orderProductItems
    .map((item) => item?.product_retailer_id ?? item?.retailer_id ?? item?.product_id ?? item?.id)
    .filter(Boolean)
    .map((retailerId) => RETAILER_ID_TO_PRODUCT_ID[retailerId])
    .filter(Boolean);
...
async function maybeSendFaqMenu(fields) {
  ...
  logEvent('outbound', `Sent Ticketing WhatsApp FAQ menu to ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
  return true;
}
...
function buildPayloads(output, fields, state = {}) {
  ...
  const deliveryMatch = output.match(MARKERS.T_DELIVERY_METHOD);
  if (deliveryMatch) {
  ...
  const seatRequestMatch = output.match(MARKERS.T_SEAT_REQUEST);
  if (seatRequestMatch) {
  ...
  const seatConfirmMatch = output.match(MARKERS.T_SEAT_CONFIRM);
  if (seatConfirmMatch) {
  ...
  const orderMatch = output.match(MARKERS.T_FOOD_ORDER_CONFIRM);
  if (orderMatch) {
    ...
    return [{
      ...
      custom: { type: 'template', template: {
        ...
        components: [
          ...
          // henry_form3's third (and final) approved component is its
          // "Complete flow" button (sub_type: flow) — same shape as
          // henry_form2's Flow button in voiceHandlers.js. Meta rejects a
          // components array that omits a component the approved template
          // actually has (error 1020 "Components sub_type invalid at
          // index: 0 and type: 0"), which is exactly what was happening
          // here: this array only ever sent header + body, silently
          // dropping the button component the live template requires. No
          // flow_action_data needed (pre-defined "Survey" screen), just a
          // unique flow_token per send, same as henry_form2.
          {
            type: 'button',
            sub_type: 'flow',
            index: 0,
  ...
  //
  // Guard: if `output` is still a raw, unhandled "[T_...]" marker at this
  // point (e.g. Claude's shared ticketingEngine deciding [T_WON], which is
  // deliberately NOT handled above for WhatsApp — see that comment), never
  // leak the literal marker text to the customer. Live-caught: Henry
  // received a bare "[T_WON]" WhatsApp message after a "Sent" button-tap
  // inbound produced that marker. Fall back to the same generic reply
  // [T_GENERIC_HELP] uses, and log it so a genuinely new unhandled marker
  // is still easy to spot in Render logs.
  if (/^\[[A-Z_]+[^\]]*\]$/.test(output.trim())) {
    console.error('Ticketing WhatsApp: unhandled marker reached the plain-text fallback, not sending it raw:', output);
    return [{ ...base, message_type: 'text', text: 'How can we help you?' }];
  }
...
async function handleSurveyCompletion(body) {
  ...
  // See the raw-shape comment above processTicketingWhatsapp's call site:
  // Vonage's button.payload is a JSON string of the Flow's screen answers
  // (there's no separate "flow name" field in this shape, unlike Meta's
  // nfm_reply — fall back to the template name for logging purposes).
  const responseJson = body.button?.payload;
  let answers = {};
  try {
    answers = responseJson ? JSON.parse(responseJson) : {};
  } catch (err) {
    console.error('Failed to parse Ticketing Flow button.payload:', err, responseJson);
  }
  saveFlowResponse({ phone, flowName: W.TEMPLATE_SURVEY, answers, receivedAt: new Date().toISOString() });
  console.log('Ticketing WhatsApp Flow response from', redactPhone(phone), JSON.stringify(answers));
  logEvent('inbound', `Received Ticketing survey (${W.TEMPLATE_SURVEY}) response from ${redactPhone(phone)}`, phone);

  const userName = getCallerName(phone) || 'there';

  // Per instruction, this demo's prize mechanism is just completing the
  // survey — no separate Instagram-tag verification step (unlike the RCS
  // demo). Reproduces the exact two-message sequence from Henry's own
  // reference WhatsApp thread: a thank-you + Instagram-tag invite text,
  // immediately followed by the "you won" reveal image. Text ported from
  // ticketingFlow.js's (RCS) equivalent [T_WON] copy for consistency.
  await sendVonageMessage({
    from: FROM, to: phone, channel: 'whatsapp', message_type: 'text',
    text: `Thank you for your feedback! 🎾 To get a chance to win tickets for next year, tag your favorite pictures on Instagram with ${T.INSTAGRAM_HASHTAG}!`,
  });
  logEvent('outbound', `Sent Ticketing WhatsApp survey thank-you/Instagram-tag invite to ${redactPhone(phone)}`, phone);
  ...
  logEvent('outbound', `Sent Ticketing WhatsApp "Your won!" to ${redactPhone(phone)}`, phone);
  ...
  for (const payload of payloads) {
    ...
    const kind =
      toSend.message_type === 'custom' && toSend.custom?.type === 'template'
        ...
        ? 'interactive message'
        : toSend.message_type;
    logEvent('outbound', `Sent Ticketing WhatsApp ${kind} to ${redactPhone(fields.clientPhone)}${auto ? ' (auto-continue after FAQ)' : ''}`, fields.clientPhone);
  }
  return { orderNumber };
}
...
async function processTicketingWhatsapp(req, res) {
  ...
  // Hoisted so the catch block below can still send a fallback message —
  // vars declared with const/let inside the try block aren't visible in
  // its catch.
  let clientPhone = '';
  ...
  try {
    ...
    // Diagnostic: kept temporarily so any future webhook-shape surprise on
    // this route is easy to read straight from Render logs.
    console.log('Ticketing WhatsApp raw inbound body:', JSON.stringify(body));

    // henry_form3's "Complete flow" button completion — confirmed live
    // (Render logs, 2026-09-01 15:46:30Z) to NOT arrive as Meta's raw
    // interactive.nfm_reply shape (that never fired). Vonage actually
    // normalizes a completed WhatsApp Flow as an ordinary button message
    // with a "flow" sub_type:
    //   { message_type: 'button', button: { text: 'Sent', sub_type: 'flow',
    //     payload: '{"screen_0_...":..., "flow_token":"..."}' } }
    // button.payload is a JSON *string* (not an object) holding the
    // screen's field values plus the flow_token — same content Meta's
    // nfm_reply.response_json would have carried, just wrapped
    // differently. Check this shape instead.
    ...
    logEvent('inbound', `Ticketing WhatsApp message from ${redactPhone(fields.clientPhone)}: "${fields.messageText}"`, fields.clientPhone);
    ...
    // to ONE_WEEK_SENT).
    if (isResetGreetingFor(DEMOS.TICKETING, fields.messageText)) {
      resetTicketingJourney(fields.clientPhone);
      logEvent('inbound', `Reset phrase detected — cleared Ticketing state for ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
    }

    const state = getTicketingState(fields.clientPhone);

    // Native WhatsApp cart checkout (message_type 'order') — the visitor
    // just submitted their [T_PRODUCT_LIST] cart. This carries structured
    // product data straight from WhatsApp's own catalog UI, so there's
    // nothing for Claude to classify and no need to walk the RCS-era
    // CHOOSING_DELIVERY/AWAITING_SEAT/CONFIRMING_SEAT sub-journey either —
    // that catalog message is headed "In-seat delivery!" specifically, so
    // the delivery method is already implied. Per Henry's instruction,
    // this goes straight to the in-seat delivery confirmation, exactly
    // like [T_FOOD_ORDER_CONFIRM: <id>:in_seat] already produces for the
    // text-driven RCS flow — reused here rather than duplicated.
    if (fields.messageType === 'order') {
      console.log('Ticketing WhatsApp raw order payload:', JSON.stringify(body.order || {}));
      if (fields.orderedProductIds.length) {
        for (const productId of fields.orderedProductIds) {
          const confirmPayloads = buildPayloads(`[T_FOOD_ORDER_CONFIRM: ${productId}:in_seat]`, fields, state);
          for (const payload of confirmPayloads) {
            await sendVonageMessage(payload);
            logEvent('outbound', `Sent Ticketing WhatsApp in-seat delivery confirmation (${productId}) to ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
          }
        }
        setTicketingState(fields.clientPhone, {
          stage: 'ORDER_PLACED',
          productId: fields.orderedProductIds[fields.orderedProductIds.length - 1],
        });
        // ORDER_PLACED has an unconditional next step (see
        // FAQ_AUTO_CONTINUE_MARKER: '[T_REVIEW]') that normally fires off
        // the visitor's next Claude-routed turn — but a cart-checkout
        // 'order' message bypasses Claude entirely and returns right
        // after this, so nothing would ever trigger it. Call the same
        // auto-continue helper the FAQ side-question path uses, so the
        // survey/prize-contest template still follows the delivery
        // confirmation on its own, matching every other stage transition
        // in this journey.
        await maybeContinueJourney(fields, getTicketingState(fields.clientPhone));
      } else {
        // Couldn't match any product_retailer_id from the order payload —
        // see extractFields' comment on the shapes it checks. Logged so
        // Henry (or I) can pull the raw payload above from Render logs
        // and fix the extraction without needing another bug report.
        logEvent('inbound', `Ticketing WhatsApp: cart/order message from ${redactPhone(fields.clientPhone)} had no recognizable product — check Render logs for the raw payload`, fields.clientPhone);
      }
      return;
    }
    ...
    if (!state.stage && !fields.faqTopicLabel) {
      ...
      logEvent('outbound', `Sent Ticketing WhatsApp template "${W.TEMPLATE_PRIORITY}" to ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
    ...
    if ((fields.messageType === 'image' || fields.messageType === 'file') && fields.mediaUrl) {
      try {
        ...
        logEvent('inbound', `Ticketing WhatsApp: could not download uploaded ticket from ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
      }
    }
    ...
    if (state.stage === 'AWAITING_SEAT' && ticketFieldsJustExtracted) {
      ...
      for (const payload of confirmPayloads) {
        await sendVonageMessage(payload);
        logEvent('outbound', `Sent Ticketing WhatsApp ${payload.message_type} to ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
      }
    ...
    logEvent('outbound', `Ticketing WhatsApp handling failed for ${redactPhone(clientPhone)}: ${err.message}`, clientPhone);
    if (clientPhone) {
      try {
        await sendVonageMessage({
          from: FROM,
        ...
        console.error('processTicketingWhatsapp fallback send also failed:', fallbackErr);
      }
    }
  }
}
