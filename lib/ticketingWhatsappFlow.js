...
async function processTicketingWhatsapp(req, res) {
  ...
  try {
    ...
    if (!state.stage && !fields.faqTopicLabel) {
      await sendVonageMessage(buildPriorityTemplate(fields.userName, fields.clientPhone));
    ...
    } else if (fields.faqTopicLabel && FAQ_AUTO_CONTINUE_MARKER[state.stage]) {
      // A pure FAQ answer (no marker, stage unchanged) at a stage whose
      // "anything else" rule is an unconditional next step — send that
      // step now too, so the FAQ side question didn't stall the journey.
      const continueMarker = FAQ_AUTO_CONTINUE_MARKER[state.stage];
      const { orderNumber: continueOrderNumber } = await sendMarkerPayloads(continueMarker, fields, state, { auto: true });
      const continueNext = computeNextState(continueMarker, { orderNumber: continueOrderNumber });
      if (continueNext.stage) {
        const { stage, ...patch } = continueNext;
        setTicketingState(fields.clientPhone, { stage, ...patch });
      }
    }
...
module.exports = { extractFields, buildPayloads, computeNextState, processTicketingWhatsapp, TICKETING_REPLY_IDS };
