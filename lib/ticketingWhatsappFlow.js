...
async function handleSurveyCompletion(body) {
  ...
  logEvent('inbound', `Received Ticketing survey (${W.TEMPLATE_SURVEY}) response from ${redactPhone(phone)}`);
  ...
  logEvent('outbound', `Sent Ticketing WhatsApp survey thank-you/Instagram-tag invite to ${redactPhone(phone)}`);
...
async function processTicketingWhatsapp(req, res) {
  ...
  try {
      ...
      if (fields.orderedProductIds.length) {
        for (const productId of fields.orderedProductIds) {
          ...
          for (const payload of confirmPayloads) {
            ...
            logEvent('outbound', `Sent Ticketing WhatsApp in-seat delivery confirmation (${productId}) to ${redactPhone(fields.clientPhone)}`);
          }
        }
        ...
        logEvent('inbound', `Ticketing WhatsApp: cart/order message from ${redactPhone(fields.clientPhone)} had no recognizable product — check Render logs for the raw payload`);
      }
      return;
    }
    ...
    logEvent('outbound', `Ticketing WhatsApp handling failed for ${redactPhone(clientPhone)}: ${err.message}`);
    if (clientPhone) {
      try {
        await sendVonageMessage({
          from: FROM,
