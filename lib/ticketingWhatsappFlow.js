...
// Matches regardless of exact punctuation/casing so "Welcome to the
// Ticketing demo!", "welcome to the ticketing demo", etc. all restart.
const RESET_PHRASE = /welcome to the ticketing demo/i;
...
async function processTicketingWhatsapp(req, res) {
  ...
  try {
    ...
    // to ONE_WEEK_SENT).
    if (RESET_PHRASE.test(fields.messageText || '')) {
