...
const { setCallContext, getCallerName, setCallerName, saveFlowResponse, resetConversation } = require('./store');
...
async function maybeTriggerFeedbackCall(fields) {
  ...
  logEvent('call', `Placed feedback call to ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
}
...
async function handleWhatsAppInbound(req, res) {
  ...
  try {
    ...
    // inbound message. Originally this only checked Meta's documented raw
    // webhook shape — an interactive message of type "nfm_reply" whose
    // response_json holds whatever fields the flow's terminal screen
    // collected. ticketingWhatsappFlow.js's own Flow completion (same
    // WABA, a different template) confirmed live (Render logs,
    // 2026-09-01) that Vonage actually normalizes a completed WhatsApp
    // Flow as an ordinary button message instead:
    //   { message_type: 'button', button: { text: 'Sent', sub_type: 'flow',
    //     payload: '{"screen_0_...":..., "flow_token":"..."}' } }
    // — meaning this check likely never matched a real henry_form2
    // completion, and it silently fell through to Claude as a plain
    // "button"/"Sent" message instead (most likely triggering [WELCOME]).
    // Checking both shapes now, same as ticketingWhatsappFlow.js's
    // handleSurveyCompletion. It's a pre-defined Meta screen (not a custom
    // flow JSON we authored), so the exact field names aren't known ahead
    // of time — stored as-is rather than mapped to named fields. This
    // isn't a conversational turn, so skip Claude's marker decision and
    // buildPayloads entirely and just capture the answers.
    const isFlowCompletion =
      (body.interactive?.type === 'nfm_reply' && body.interactive?.nfm_reply) ||
      (body.message_type === 'button' && body.button?.sub_type === 'flow');
    if (isFlowCompletion) {
      const phone = body.from ?? '';
      const responseJson = body.interactive?.nfm_reply?.response_json ?? body.button?.payload;
      const flowName = body.interactive?.nfm_reply?.name || 'henry_form2';
      let answers = {};
      try {
        answers = responseJson ? JSON.parse(responseJson) : {};
      } catch (err) {
        console.error('Failed to parse Flow response:', err, responseJson);
      }
      saveFlowResponse({ phone, flowName, answers, receivedAt: new Date().toISOString() });
      console.log('WhatsApp Flow response from', redactPhone(phone), JSON.stringify(answers));
      logEvent('inbound', `Received Survey (${flowName}) response from ${redactPhone(phone)}`, phone);
      return;
    }
    ...
    // number left behind last time it was tested (confirmed live on the RCS
    // side: a fresh greeting produced [TIMESLOT_LIST] instead of [WELCOME]
    // because the number's prior test had already reached "Book a
    // viewing") — applied here too for parity, same shared conversation
    // store and marker engine.
    if (isResetGreetingFor(DEMOS.REAL_ESTATE, fields.messageText)) {
      resetConversation(fields.clientPhone, 'real-estate');
      logEvent('inbound', `Reset phrase detected — cleared Real Estate conversation history for ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
    }

    console.log('WhatsApp inbound fields:', JSON.stringify(fields));
    logEvent('inbound', `WhatsApp message from ${redactPhone(fields.clientPhone)}: "${fields.messageText}"`, fields.clientPhone);
    ...
    for (const payload of payloads) {
      ...
      logEvent('outbound', `Sent ${kind} to ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
    }

    // Henry: disabled by request — replying YES/NO to a viewing
    // confirmation was placing an unwanted outbound "feedback" call a
    // few seconds later (maybeTriggerFeedbackCall below), which in turn
    // triggered the "Thank you for your feedback!" follow-up message
    // once that call completed (see voiceHandlers.js's
    // sendFeedbackThankYou, gated on context.context === 'feedback_call'
    // — never reached now that no call is placed). Left the function
    // itself intact in case this is wanted back later.
    // await maybeTriggerFeedbackCall(fields);
  } catch (err) {
    console.error('handleWhatsAppInbound error:', err);
    logEvent('outbound', `Real Estate WhatsApp handling failed for ${redactPhone(clientPhone)}: ${err.message}`, clientPhone);
    // Previously this just returned, leaving the visitor's phone showing
    // nothing with no indication anything went wrong — the most common
    // real cause is decideMarker() (Claude) or sendVonageMessage (Vonage)
    // timing out. A one-line fallback at least makes the failure visible
    // during a live demo instead of silent.
    if (clientPhone) {
      try {
        ...
        console.error('handleWhatsAppInbound fallback send also failed:', fallbackErr);
      }
    }
  }
}
