...
const { getConversation, pushTurn, setCallContext, getCallerName, setCallerName } = require('./store');
...
// Henry's Real Estate demo!" — lets us capture the first name the visitor
// typed in without needing a separate session/lookup mechanism. If the
// visitor edits the pre-filled text before sending, this simply won't
// match and personalization falls back to WhatsApp's profile name.
...
async function handleWhatsAppInbound(req, res) {
  ...
  try {
    ...
    const greetingMatch = fields.messageText.match(DEMO_GREETING_RE);
    if (greetingMatch) {
      const name = capitalize(greetingMatch[1].trim());
      setCallerName(fields.clientPhone, name);
