...
async function handleWhatsAppInbound(req, res) {
  ...
  try {
    ...
    if (isFlowCompletion) {
      ...
      logEvent('inbound', `Received Survey (${flowName}) response from ${redactPhone(phone)}`);
      return;
    }
    ...
    logEvent('outbound', `Real Estate WhatsApp handling failed for ${redactPhone(clientPhone)}: ${err.message}`);
