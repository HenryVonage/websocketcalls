...
function attachVoiceBridge(vonageWs) {
  ...
  maxDurationTimer = setTimeout(() => {
    ...
    if (vonageWs.readyState === WebSocket.OPEN) vonageWs.close();
      ...
      try {
        // The "Contact us" voice escalation from the Ticketing demo lands
        // here too (same Vonage Application, same /voice websocket) —
        ...
        // agent if a dedicated ticketing agent hasn't been created yet in
        // ElevenLabs / configured via ELEVENLABS_TICKETING_AGENT_ID.
        ...
        const agentId = (isTicketing && process.env.ELEVENLABS_TICKETING_AGENT_ID) || process.env.ELEVENLABS_AGENT_ID;
