...
// Lovers call connects, then duck it under the agent's opening for however
// much of the clip is left, before the agent carries on solo. The 2s solo
// window doubles as cover for the ElevenLabs connect/first-response
// latency that's there anyway (see the pacing comment above), rather than
// adding a new delay.
//
// Sept 2026 update, Henry: two live-call reports after the first version
// of this feature shipped — (1) "the beginning of the agent first 3 sec
// talk is still breaking", and (2) "make the background music slightly
// louder and last 7 sec in total". (1) traced back to a real gap: the
// original version queued a *fixed* 2s of solo audio and then immediately
// treated the call as "mixing", trusting that ElevenLabs' own audio would
// already be arriving by the time that 2s drained. ElevenLabs' actual
// connect + first-token latency isn't guaranteed to beat 2s — when it
// didn't, audioQueue ran dry (nothing left to send) until the agent's
// audio finally showed up, which is exactly the kind of gap that sounds
// like "breaking" on a call. Fixed below by tracking a single cursor into
// the resolved clip (introClipCursor) instead of a fixed solo/background
// split: the guaranteed INTRO_SOLO_MS still plays solo up front, but if
// the agent still hasn't spoken by the time that drains, startAudioPacing's
// tick() keeps topping the queue up with more (still-solo) clip audio a
// couple hundred ms at a time — see FILLER_LOW_WATER_MS/FILLER_TOPUP_MS
// below — so playback never goes silent while waiting. The moment real
// agent audio arrives (appendMixedOrPassthrough), mixing picks up from
// wherever the cursor happens to be, with no gap and nothing replayed
// twice. (2) is just CALL_INTRO_SECONDS (ringtoneBuilder.js) going from 5
// to 7 and INTRO_BACKGROUND_GAIN below going up — the duck-under window is
// now simply "whatever's left of the clip after the 2s solo guarantee", so
// it stretches to match automatically.
//
...
// (full-volume) audio — roughly -11dB, audible as background without
// competing with the voice. Bumped up from an earlier, quieter 0.18
// (~-15dB) per Henry's "slightly louder" feedback above.
const INTRO_BACKGROUND_GAIN = 0.28;
// If the agent hasn't started talking yet and the queue is about to run
// dry, top it up with more (still-solo) clip audio rather than letting
// startAudioPacing's tick() go silent — see the Sept 2026 update comment
// above. The low-water mark is comfortably bigger than one 20ms frame so a
// single slow tick can't itself trigger a top-up thrash.
const FILLER_LOW_WATER_MS = 300;
const FILLER_TOPUP_MS = 200;
...
function attachVoiceBridge(vonageWs) {
  ...
  // in pendingAgentAudio) -> 'active' (clip resolved: solo portion already
  // queued, and either being topped up with more solo/filler clip audio or
  // mixed under live agent audio, depending on introAgentActive) -> 'done'
  // (clip exhausted, or bailed out — plain passthrough from here).
  // Modeled as an explicit phase rather than a couple of booleans since
  // 'audio'/'interruption' events need to branch on it from more than one
  // place below.
  let introPhase = 'none';
  let pendingAgentAudio = Buffer.alloc(0);
  // The resolved clip and a single cursor into it, replacing an earlier
  // fixed solo/background byte split — see the Sept 2026 update comment
  // above MUSIC_INTRO_ENABLED for why a cursor is what closes the gap.
  let introClip = null;
  let introClipCursor = 0;
  // False until the agent's first real (post-resolution) audio chunk
  // arrives — gates the tick()-driven filler top-up off once there's live
  // agent audio to mix against instead.
  let introAgentActive = false;
  ...
  async function resolveIntroClip(phone) {
    ...
    if (!state.spotifyTrackId) {
      // The other half of the "native call vs. Call-on-WhatsApp button"
      // diagnosis above — this is what "no intro" looks like when the call
      // *was* correctly recognized as Music Lovers but this phone number's
      // journey state has no matched track yet (e.g. the caller never
      // finished the genre-picker step in the current run, or a fresh
      // greeting reset it since). Silent otherwise (this is a normal,
      // expected outcome for a genuinely track-less caller), but worth a
      // log line given how easy this and the context-mismatch cause above
      // are to conflate from the caller's side — both just sound like "no
      // music played".
      console.log('Music call-intro: no matched track on file for this phone yet — skipping intro. state:', JSON.stringify(state));
      return null;
    }
    let previewUrl = null;
    try {
  ...
  // Mixes one chunk of agent audio against whatever's left of introClip
  // from introClipCursor onward, or passes it straight through once the
  // clip is exhausted. Called both for the backlog that piled up in
  // pendingAgentAudio while we waited on the clip, and for every live
  // 'audio' event afterwards until the clip runs out. Marks
  // introAgentActive so startAudioPacing's filler top-up (tick(), below)
  // stops trying to fill on our behalf — there's live audio to mix now.
  function appendMixedOrPassthrough(agentBytes) {
    introAgentActive = true;
    if (introPhase !== 'active' || !introClip || introClipCursor >= introClip.length) {
      audioQueue = Buffer.concat([audioQueue, agentBytes]);
      if (introPhase === 'active') { introPhase = 'done'; introClip = null; }
      return;
    }
    ...
    const remaining = introClip.length - introClipCursor;
    const mixLen = Math.min(agentBytes.length, remaining) & ~1;
    const mixed = mixPcm16(agentBytes.subarray(0, mixLen), introClip.subarray(introClipCursor, introClipCursor + mixLen), INTRO_BACKGROUND_GAIN);
    audioQueue = Buffer.concat([audioQueue, mixed]);
    introClipCursor += mixLen;
    if (mixLen < agentBytes.length) {
      // Clip ran out mid-chunk — the rest of this chunk plays solo.
      audioQueue = Buffer.concat([audioQueue, agentBytes.subarray(mixLen)]);
    }
    if (introClipCursor >= introClip.length) {
      introPhase = 'done';
      introClip = null;
    }
  }

  // Fires once resolveIntroClip settles. Queues the guaranteed solo portion
  // immediately (audioQueue should still be empty at this point in the
  // ordinary case — the call just connected) and advances introClipCursor
  // past it, then mixes in whatever agent audio already arrived while we
  // were waiting. If nothing arrived yet, introClip/introClipCursor are
  // left in place for startAudioPacing's tick() to top up from (see the
  // Sept 2026 update comment above MUSIC_INTRO_ENABLED) until either the
  // agent starts talking or the clip runs out.
  ...
  function startAudioPacing() {
    const fillerLowWaterBytes = FILLER_LOW_WATER_MS * BYTES_PER_MS;
    const fillerTopupBytes = Math.round(FILLER_TOPUP_MS * BYTES_PER_MS) & ~1;
    let nextTickAt = Date.now();
    const tick = () => {
      nextTickAt += FRAME_INTERVAL_MS;
      // Music Lovers call-intro filler: if the clip has resolved, the agent
      // hasn't started talking yet, and the queue is about to run dry, top
      // it up with more (still-solo) clip audio instead of letting this
      // tick go silent — see the Sept 2026 update comment above
      // MUSIC_INTRO_ENABLED for why this is here.
      if (introPhase === 'active' && !introAgentActive && introClip && introClipCursor < introClip.length && audioQueue.length < fillerLowWaterBytes) {
        const topUpLen = Math.min(fillerTopupBytes, introClip.length - introClipCursor);
        audioQueue = Buffer.concat([audioQueue, introClip.subarray(introClipCursor, introClipCursor + topUpLen)]);
        introClipCursor += topUpLen;
        if (introClipCursor >= introClip.length) {
          introPhase = 'done';
          introClip = null;
        }
  ...
  maxDurationTimer = setTimeout(() => {
    ...
    logEvent('call', `Call with ${redactPhone(meta.callerPhone)} ended: reached ${MAX_CALL_DURATION_MS / 60000}-minute max duration`, meta.callerPhone);
  ...
  startAudioPacing();
      ...
      // Sept 2026, Henry: "if I run an inbound call directly from the
      // calling feature of the WABA, that sound in the background is not
      // played. It seems to work only when using the 'Call on WhatsApp'
      // button." Both paths hit this exact same /answer webhook and the
      // exact same code here — there is no separate code path for "native
      // call icon" vs. "template's Call button" on our side. meta.context
      // itself comes from voiceHandlers.js's detectDemo(), which (for this
      // shared WABA number) falls back to whichever demo this caller's
      // phone number was last *texted*-bound to (store.js's activeDemos —
      // see demoRouter.js) — a native call carries no signal of its own, so
      // if that binding had drifted to a different demo since the last
      // Music Lovers text (e.g. testing Ticketing/Real Estate in between),
      // meta.context won't start with 'music_lovers' at all and everything
      // below is skipped, silently. This log is the fastest way to tell
      // that apart from the other possible cause (resolveIntroClip finding
      // no matched track — see its own log below) next time this happens —
      // check Render logs for this line right after the "not connected"
      // console.log above.
      console.log('Music call-intro gate — context:', meta.context, 'isMusicLovers:', isMusicLovers, 'MUSIC_INTRO_ENABLED:', MUSIC_INTRO_ENABLED);
