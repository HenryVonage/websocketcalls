...
function attachVoiceBridge(vonageWs) {
  ...
  vonageWs.on('message', async (data, isBinary) => {
    if (!isBinary) {
      ...
      console.log('Voice call connected. Context:', meta.context, 'caller:', meta.callerPhone);
      ...
      elevenWs.on('message', (raw) => {
        ...
        switch (event.type) {
          case 'conversation_initiation_metadata': {
            const fmt = event.conversation_initiation_metadata_event;
            console.log('ElevenLabs audio formats — agent output:', fmt?.agent_output_audio_format, 'user input:', fmt?.user_input_audio_format);
            break;
          }
          case 'audio': {
