...
const { getSeed } = require('./ownerSeed');

const PARALLEL = 4;
const PREVIEW_SOURCE = String(process.env.PREVIEW_SOURCE || 'none').toLowerCase();
...
// ---------- iTunes preview ----------
// Apple throttles the unauthenticated search/lookup API per IP (their
// docs say ~20 calls/min; in practice a few per second is tolerated but
// a burst gets 403 "Forbidden" HTML for a while). The first live
// pre-analysis of Henry's library (Sept 2026) fired ~12 lookups/s, got
// throttled within seconds, and — because a 403 looked like "no result"
// — cached ~900 tracks as "no preview" for good. So now: every iTunes
// call goes through a global pacer (ITUNES_MIN_GAP_MS apart), a 403/429
// waits ITUNES_BACKOFF_MS and retries once, and if it's *still* throttled
// the lookup throws Throttled so getFeatures leaves the track uncached
// for the next run instead of remembering a false "nothing found".
const ITUNES_MIN_GAP_MS = 350; // ≈170 calls/min across all workers
const ITUNES_BACKOFF_MS = 65000;
let itunesNextSlot = 0;
class Throttled extends Error {}

async function itunesGet(url) {
  for (let attempt = 0; attempt < 2; attempt++) {
    const wait = Math.max(0, itunesNextSlot - Date.now());
    itunesNextSlot = Math.max(itunesNextSlot, Date.now()) + ITUNES_MIN_GAP_MS;
    if (wait) await new Promise((r) => setTimeout(r, wait));
    const res = await fetchWithTimeout(url);
    if (res.status === 403 || res.status === 429) {
      if (attempt === 0) {
        console.error(`trackFeatures: iTunes ${res.status} (throttled) — pausing ${ITUNES_BACKOFF_MS / 1000}s`);
        itunesNextSlot = Date.now() + ITUNES_BACKOFF_MS;
        continue;
      }
      throw new Throttled(`iTunes ${res.status} after backoff`);
    }
...
async function lookupItunesPreview({ isrc, title, artist }) {
  let json = null;
  if (isrc) json = await itunesGet(`https://itunes.apple.com/lookup?isrc=${encodeURIComponent(isrc)}&entity=song`);
  if (!json || !json.resultCount) {
    ...
    json = await itunesGet(`https://itunes.apple.com/search?term=${term}&entity=song&limit=5`);
  }
...
async function getFeatures(track) {
  ...
  let previewUrl = null;
  let throttled = false;
  // Sept 2026: previews are OFF unless PREVIEW_SOURCE=itunes. Henry wants
  // the demo built on Spotify (plus Spotify's own tempo/key seed) — no
  // Apple lookups, and the beat-matched teaser they fed is retired
  // (lib/musicLoversMix.js). Tempo / key come from the seed, or from
  // GetSongBPM when GETSONGBPM_API_KEY is set; energy defaults to the
  // seed's or stays unknown (the planner treats unknown as mid).
  if (PREVIEW_SOURCE === 'itunes') {
    try {
      previewUrl = await lookupItunesPreview(track);
    } catch (err) {
      throttled = err instanceof Throttled;
      if (!throttled) console.error(`trackFeatures: iTunes lookup failed for ${track.title}:`, err.message);
    }
  }
  // Spotify's own (pre-Nov-2024) tempo/key for Henry's library, exported
  // to data/owner-track-seed.json — the most trustworthy source when we
  // have it, so it's checked before GetSongBPM (lib/ownerSeed.js).
  features = getSeed(track.id);
  if (!features) {
    try {
  ...
  // A throttled lookup is not an answer: don't remember "nothing found",
  // so the next run (or the next mix) tries the preview again. Seed /
  // GetSongBPM tempo is still returned for this call.
  if (!throttled || previewUrl) cacheSet(track.id, features || null);
