...
// Turns an ElevenLabs conversation transcript into a short, friendly
// message recap for the caller, using Claude. `options.businessName`
// defaults to 'Vonage Estate' (unchanged behavior for every existing call
// site); the Ticketing demo's voice follow-up (voiceHandlers.js) passes the
// event name instead so the recap reads correctly for that call.
async function summarizeTranscript(transcript, options = {}) {
  const { businessName = 'Vonage Estate' } = options;
  const lines = (transcript || [])
    .map((turn) => `${turn.role === 'agent' ? 'Assistant' : 'Caller'}: ${turn.message}`)
    .join('\n');

  if (!lines.trim()) {
    return `Thanks for calling ${businessName}! We didn't catch much detail this time, but a member of our team will follow up shortly.`;
  }

  const response = await anthropic.messages.create({
    model: 'claude-sonnet-4-6',
    max_tokens: 200,
    system: `Summarize this ${businessName} phone call transcript into a short, friendly message recap for the caller (this may be sent over WhatsApp or RCS). Maximum ${MAX_CHARS} characters, including spaces. Write it directly to the caller (second person), warm and concise, no headers or bullet points, plain text only.`,
...
// Music Lovers counterpart to analyzeBookingIntent above — decides whether
// an inbound caller expressed interest in booking concert tickets during
// the call (Sept 2026, Henry's request: "if the user has ordered concert
// tickets > send the templated message 'henry_ticketing2'"). Deliberately
// does NOT ask Claude for the artist/venue/date the way analyzeBookingIntent
// asks for propertyName/appointmentTime — those are already reliably known
// from musicLoversState (set when henry_ticketingconcert was originally
// sent, see musicLoversFlow.js's sendConcertsTemplate), so re-extracting
// them from the transcript would just be a second, less reliable source for
// the same facts. Runs off the same transcript already fetched for the
// recap — no extra ElevenLabs call.
async function analyzeTicketIntent(transcript) {
  const lines = (transcript || [])
    .map((turn) => `${turn.role === 'agent' ? 'Assistant' : 'Caller'}: ${turn.message}`)
    .join('\n');

  if (!lines.trim()) return { wantsTickets: false };

  const response = await anthropic.messages.create({
    model: 'claude-sonnet-4-6',
    max_tokens: 200,
    system: `Read this Vonage Music Lovers phone call transcript. Decide whether the caller wants to book/order concert tickets. Respond with ONLY a JSON object, no other text, exactly matching this shape:
{"wantsTickets": boolean, "name": string}

- wantsTickets: true only if the caller clearly agreed to, asked for, or confirmed ordering tickets to a concert.
- name: the caller's first name if mentioned in the conversation, otherwise "there".
Fill both fields with your best guess even if wantsTickets is false.`,
    messages: [{ role: 'user', content: lines }],
  });
  ...
  try {
    ...
    console.error('Failed to parse ticket intent JSON:', raw, err.message);
    return { wantsTickets: false };
  }
}
