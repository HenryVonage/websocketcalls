...
async function initStore() {
  ...
  await Promise.all([
    ...
    hydrateMap('trackFeatures', trackFeatures),
    hydrateMap('mixPlans', mixPlans),
    hydrateMap('topTracksCatalog', topTracksCatalog),
    hydrateMap('musicTickets', musicTickets),
    hydrateMap('musicTicketsByKey', musicTicketsByKey),
...
// BPM / key / energy per Spotify track id, filled by lib/trackFeatures.js
// for the "Personal DJ mix" feature (see lib/musicLoversMix.js). A track
// analysed once for any listener is reused for every later listener —
// this is the expensive step (preview download + analysis, ~1 s each),
// so it's persisted with the rest of the store. `null` is a valid value
// ("looked everywhere, found nothing") and is cached too, so a track
// with no preview isn't retried on every mix. Capped: 8 000 entries is
// ~1.6 MB of JSON, comfortably inside a single Redis string (raised from
// 5 000 once Henry's whole library — ~2 000+ tracks — was pre-analysed).
const trackFeatures = new Map();

function getTrackFeatures(trackId) {
  return trackFeatures.get(trackId); // undefined = never looked up
}

function setTrackFeatures(trackId, features) {
  if (!trackId) return;
  trackFeatures.set(trackId, features);
  capMapSize(trackFeatures, 8000);
  persistMap('trackFeatures', trackFeatures);
}

// Drops cached "nothing found" (null) entries — and entries with no
// preview — for the given ids so they get looked up again. Used by the
// owner-library pre-analysis with ?retry=1 after the Sept 2026 iTunes
// throttling episode cached hundreds of false negatives. Returns the
// number forgotten.
function forgetMissingTrackFeatures(trackIds) {
  let n = 0;
  for (const id of trackIds) {
    if (!trackFeatures.has(id)) continue;
    const f = trackFeatures.get(id);
    if (f === null || !f.previewUrl) {
      trackFeatures.delete(id);
      n++;
    }
  }
  if (n) persistMap('trackFeatures', trackFeatures);
  return n;
}

// Planned sets for the "Personal DJ mix" feature, keyed by a random setId
// baked into the teaser audio URL WhatsApp fetches — kept so a fetch that
// lands after a Render restart can re-render from the plan instead of
// 404ing (lib/musicLoversMix.js renderTeaserForSet). Small and
// short-lived, capped at 200.
const mixPlans = new Map();

function setMixPlan(setId, entry) {
  if (!setId) return;
  mixPlans.set(setId, entry);
  capMapSize(mixPlans, 200);
  persistMap('mixPlans', mixPlans);
}

function getMixPlan(setId) {
  return mixPlans.get(setId);
}
...
// Fake per-artist concert ticket generated for a Music Lovers caller who
// expressed ticket-booking intent on an inbound voice call (Sept 2026,
// Henry's request — see voiceHandlers.js's sendMusicLoversCallFollowUp).
// Kept in two maps of the SAME record, same tradeoff as callContext/
// callSummaries above:
//  - by phone, so the (not yet built) "My tickets over WhatsApp" button-tap
//    handler can look up this caller's most recent ticket — a button tap
//    only ever carries the phone number, not any ticket-specific id.
//  - by an opaque ticketKey, so the ticket PDF route (server.js) doesn't
//    need the phone number in its public URL. Deliberately a separate
//    random value from the orderNumber shown in the WhatsApp message body —
//    the order number is only 6 digits (fine to *show* the caller, since
//    it's a fake demo order number either way) but too easily brute-forced
//    to also use as an unguessable PDF-fetch token.
const musicTickets = new Map(); // by phone
const musicTicketsByKey = new Map(); // by ticketKey
const MAX_TRACKED_MUSIC_TICKETS = 300;

function setMusicTicketDetails(phone, data) {
  if (!phone || !data?.ticketKey) return;
  const record = { ...data, generatedAt: Date.now() };
  musicTickets.set(phone, record);
  musicTicketsByKey.set(data.ticketKey, record);
  capMapSize(musicTickets, MAX_TRACKED_MUSIC_TICKETS);
  capMapSize(musicTicketsByKey, MAX_TRACKED_MUSIC_TICKETS);
  persistMap('musicTickets', musicTickets);
  persistMap('musicTicketsByKey', musicTicketsByKey);
  return record;
}

function getMusicTicketDetails(phone) {
  return musicTickets.get(phone);
}

function getMusicTicketByKey(ticketKey) {
  return musicTicketsByKey.get(ticketKey);
}
