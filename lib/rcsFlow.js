...
const { resolveDemo, DEMOS, isResetGreetingFor } = require('./demoRouter');
const { extractFields: ticketingExtractFields, processTicketingRcs } = require('./ticketingFlow');
const { extractFields: musicLoversExtractFields, processMusicLoversRcs } = require('./musicLoversRcsFlow');
...
function extractFields(body) {
  ...
  // are used for two different conversational roles depending on which
  // WhatsApp control they mirror, and the shared conversationEngine's
  // prompt tells them apart by message_type:
  //   - 'button' for WhatsApp quick-reply BUTTON taps (STEPs
  //     2A/2B/6/7/8/9 — the welcome menu and the booking/decision chips)
  //   - 'reply' for WhatsApp interactive LIST-style picks (STEP 3's
  //     flat-type/budget choice, STEP 6B's timeslot choice)
  // Both are implemented as the very same RCS 'reply' suggestion chip on
  // our side, so this exact-label list is what tells them apart. Getting
  // it wrong doesn't error — it silently falls through to the prompt's
  // "if in doubt, [WELCOME]" default instead of the intended next step.
  // That's exactly what was happening: picking a viewing timeslot has no
  // matching STEP under message_type 'button' (STEP 6B needs 'reply'),
  // so the conversation reset to the welcome card instead of continuing
  // to the video tour/confirmation.
  const BUTTON_STYLE_REPLIES = new Set([
    'Flats available near by',
    'Viewing',
    'BOOK A VIEWING',
    'NOT INTERESTED',
    'QUESTIONS ABOUT THE FLAT',
    'YES',
    'NO',
  ]);
  const messageType = rawType === 'reply'
    ? (BUTTON_STYLE_REPLIES.has(body.reply?.id) ? 'button' : 'reply')
    : rawType;
...
async function maybeTriggerFeedbackCall(fields) {
  ...
  logEvent('call', `Placed feedback call to ${redactPhone(fields.clientPhone)} (RCS demo)`, fields.clientPhone);
}
...
async function handleRcsInbound(req, res) {
  ...
  // its catch. isTicketing/isMusicLovers stay false unless we actually hand
  // off to that demo, each of which has its own internal error
  // handling/fallback — so this function's own fallback below only fires
  // for its own (Real Estate) errors, not a bug in one of those that
  // happened to bubble up here.
  let clientPhone = '';
  let isTicketing = false;
  let isMusicLovers = false;
  ...
  try {
    ...
    if (demo === DEMOS.MUSIC_LOVERS) {
      isMusicLovers = true;
      await processMusicLoversRcs(musicLoversExtractFields(body));
      return;
    }
    ...
    logEvent('inbound', `RCS message from ${redactPhone(fields.clientPhone)}: "${fields.messageText}"`, fields.clientPhone);
    ...
    for (const payload of payloads) {
      ...
      logEvent('outbound', `Sent RCS ${kind} to ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
    }
    ...
    logEvent('outbound', `Real Estate RCS handling failed for ${redactPhone(clientPhone)}: ${err.message}`, clientPhone);
    if (clientPhone && !isTicketing && !isMusicLovers) {
      try {
        ...
        console.error('handleRcsInbound fallback send also failed:', fallbackErr);
      }
    }
  }
}
