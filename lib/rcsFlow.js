...
// (conversationEngine.js) as whatsappFlow.js â only the message-building
...
// equivalent approval UI for RCS â these payloads are built directly from
...
// for the WhatsApp templates â check the activity log (/api/logs) for
...
function extractFields(body) {
  ...
  // play the same conversational role as WhatsApp's quick-reply buttons â
  ...
  // property ID directly (e.g. "L1234") â see buildRcsPayloads's
...
function buildPayloads(output, fields) {
    ...
    return [{
      ...
      card: {
        ...
        suggestions: [
          ...
          // straight from the welcome card too â forwards over PSTN to
  ...
  if (output.includes('[VOICE_CALL]')) {
    // No "calling within RCS" concept â points the caller at the real
  ...
  if (output.includes('[PRODUCT_LIST]')) {
    // Two standalone cards rather than a carousel â Vonage's docs don't
  ...
  if (output.includes('[FLAT_TYPE_LIST]')) {
    ...
    // chips â RCS supports up to 11 chips below a message. Chip text is
    // capped at 25 characters by RCS, so labels are abbreviated.
    return [{
      from: FROM, to: clientPhone, channel: 'rcs', message_type: 'text',
      text: 'What type of flat, and what budget?',
      suggestions: [
        { type: 'reply', text: 'Studio <Â£1000', postback_data: 'studio_less1000' },
        { type: 'reply', text: 'Studio Â£1-1.5k', postback_data: 'studio_1000_1500' },
        { type: 'reply', text: 'Studio >Â£1500', postback_data: 'studio_over1500' },
        { type: 'reply', text: '1bd <Â£1500', postback_data: 'onebed_less1500' },
        { type: 'reply', text: '1bd Â£1.5-2k', postback_data: 'onebed_1500_2000' },
        { type: 'reply', text: '1bd >Â£2000', postback_data: 'onebed_over2000' },
        { type: 'reply', text: '2bd <Â£1800', postback_data: 'twobed_less1800' },
        { type: 'reply', text: '2bd Â£1.8-2.2k', postback_data: 'twobed_1800_2200' },
        { type: 'reply', text: '2bd >Â£2200', postback_data: 'twobed_over2200' },
      ],
    }];
  }

  // The user just tapped "Book a viewing" â offer a few concrete slots
    ...
    return [
      {
        from: FROM, to: clientPhone, channel: 'rcs', message_type: 'card',
        card: {
          title: `${propertyName} â video tour`,
      ...
      },
      {
        ...
        card: {
          ...
          suggestions: [
            ...
            // Same dial suggestion as [WELCOME]/[VOICE_CALL] â WhatsApp's
  ...
  if (output.includes('[VIDEO_TOUR')) {
    ...
    // actually picked from [TIMESLOT_LIST] through as "name|slot" â fall
...
// RCS equivalent of whatsappFlow.js's maybeTriggerFeedbackCall â RCS has
...
async function handleRcsInbound(req, res) {
  ...
  try {
    ...
    // Vonage Application, RCS agent, and number â so every inbound RCS
    ...
    // before doing anything else â see lib/demoRouter.js. Falls back to
    ...
    if (isResetGreeting(fields.messageText)) {
      ...
      logEvent('inbound', `Reset phrase detected â cleared Real Estate conversation history for ${redactPhone(fields.clientPhone)}`);
    }
    ...
    logEvent('decision', `Marker decision (RCS): ${output.length > 80 ? output.slice(0, 80) + 'â¦' : output}`);

    // STEP 3B (conversationEngine.js): the "share_location" chip tap
    // itself, not actual location data â nothing to send back. Without
