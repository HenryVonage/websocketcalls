...
async function handleRcsInbound(req, res) {
  ...
  // its catch. isTicketing stays false unless we actually hand off to the
  // Ticketing demo, which has its own internal error handling/fallback —
  // so this function's own fallback below only fires for its own (Real
  // Estate) errors, not a Ticketing bug that happened to bubble up here.
  let clientPhone = '';
