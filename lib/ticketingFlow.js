...
function buildPayloads(output, fields, state = {}) {
  ...
  if (output.includes('[T_PRODUCT_LIST]')) {
    // Sent as one real RCS carousel (Google's native RBM richCard.
    // carouselCard schema, reached via Vonage's message_type: 'custom'
    // passthrough) rather than a stack of standalone cards -- confirmed
    // against two independent sources rather than guessed: the original
    // Node-RED flow this demo is rebuilt from (flows_24.json's "Multiple
    // Products" function node used exactly this shape successfully) and
    // Vonage's own published RCS-carousel Node.js sample
    // (github.com/Vonage-Community/tutorial-messages-node-rcs_carousel-rich-card).
    // suggestions use { reply: {...} } (a plain suggested reply chip),
    // matching flows_24.json's usage -- Vonage's own sample additionally
    // shows { action: {...} } for open_url/calendar-type suggestions,
    // which this carousel doesn't need.
    return [{
      to: base.to,
      from: base.from,
      channel: 'rcs',
      ...
      custom: {
        contentMessage: {
          richCard: {
            carouselCard: {
              cardWidth: 'MEDIUM',
              cardContents: T.PRODUCTS.map((p) => ({
                title: `${p.name} — ${p.price}`,
                description: 'Tap to order this for in-seat delivery or stand collection.',
                media: {
                  height: 'MEDIUM',
                  contentInfo: { fileUrl: p.imageUrl, forceRefresh: false },
                },
                suggestions: [{ reply: { text: p.name, postbackData: p.name } }],
              })),
            },
          },
        },
...
async function processTicketingRcs(fields) {
  ...
  // Previously missing entirely on this (RCS) path — the WhatsApp side of
  // Ticketing has always had its own reset check, but re-scanning the
  // Ticketing RCS QR code on a phone number that had already advanced
  // (e.g. to DDAY_SENT) just resumed that stale progress instead of
  // restarting, the same bug class fixed for Music Lovers (Sept 2026).
  if (isResetGreetingFor(DEMOS.TICKETING, fields.messageText)) {
    resetTicketingJourney(fields.clientPhone);
    logEvent('inbound', `Reset phrase detected — cleared Ticketing RCS state for ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
  }

  const state = getTicketingState(fields.clientPhone);

  try {
    // First-ever contact for this phone number in this demo: always send
    // the "priority access" card, deterministically, with no Claude call.
    // This is the exact first outbound the recorded demo showed, so it
    // must never depend on the model classifying a greeting correctly.
    if (!state.stage) {
      const payload = buildPriorityCard(fields.userName);
      await sendVonageMessage({ ...payload, to: fields.clientPhone });
      logEvent('outbound', `Sent Ticketing RCS card "${payload.card.title}" to ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
    ...
    if ((fields.messageType === 'image' || fields.messageType === 'file') && fields.mediaUrl) {
      try {
        ...
        logEvent('inbound', `Ticketing: could not download uploaded ticket from ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
      }
    }
    ...
    logEvent('inbound', `Ticketing RCS message from ${redactPhone(fields.clientPhone)}: "${fields.messageText}"`, fields.clientPhone);
    ...
    for (const payload of payloads) {
      ...
      logEvent('outbound', `Sent Ticketing RCS ${kind} to ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
    }
    ...
    if (state.stage === 'AWAITING_SEAT' && ticketFieldsJustExtracted) {
      ...
      for (const payload of confirmPayloads) {
        await sendVonageMessage(payload);
        logEvent('outbound', `Sent Ticketing RCS ${payload.message_type} to ${redactPhone(fields.clientPhone)}`, fields.clientPhone);
      }
      setTicketingState(fields.clientPhone, { stage: 'CONFIRMING_SEAT', productId, row, seat });
    }
  } catch (err) {
    // Previously any error here (most commonly decideTicketingMarker/
    // Claude, or sendVonageMessage, timing out) propagated up to
    // rcsFlow.js's own catch and vanished silently — the visitor's phone
    // just showed nothing. A one-line fallback at least makes the failure
    // visible during a live demo.
    console.error('processTicketingRcs error:', err);
    logEvent('outbound', `Ticketing RCS handling failed for ${redactPhone(fields.clientPhone)}: ${err.message}`, fields.clientPhone);
    try {
      ...
      console.error('processTicketingRcs fallback send also failed:', fallbackErr);
    }
  }
}
