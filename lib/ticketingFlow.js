...
async function processTicketingRcs(fields) {
  ...
  if (isResetGreetingFor(DEMOS.TICKETING, fields.messageText)) {
    ...
    logEvent('inbound', `Reset phrase detected — cleared Ticketing RCS state for ${redactPhone(fields.clientPhone)}`);
  }
  ...
  try {
    ...
    logEvent('outbound', `Ticketing RCS handling failed for ${redactPhone(fields.clientPhone)}: ${err.message}`);
    try {
