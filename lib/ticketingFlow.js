...
function buildPriorityCard(userName) {
  return {
    ...
    card: {
      ...
      media_height: 'TALL',
      suggestions: [
        { type: 'open_url', text: 'Online ticketing', postback_data: 'online_ticketing', url: T.TICKETING_SITE_URL },
        { type: 'dial', text: 'Call', postback_data: 'call_us', phone_number: `+${config.RCS_PSTN_NUMBER}` },
        { type: 'open_url', text: 'Info', postback_data: 'info', url: T.INFO_URL },
      ],
    },
  ...
  if (output.includes('[T_WON]')) {
    return [{
      ...
      card: {
        ...
        media_height: 'TALL',
        suggestions: [
          { type: 'open_url', text: 'How to receive my tickets', postback_data: 'how_to_receive', url: 'https://montecarlotennismasters.com/en/tickets/' },
        ],
      },
      rcs: { card_orientation: 'VERTICAL' },
    }];
