...
// Club arc shapes as fractions of total length; BPM offsets are relative to
// the pool median and clamped into the pool's own 10th–90th percentile so a
// narrow pool (all 124–126) doesn't get asked for tempos it can't supply.
function buildArc(totalSec, poolBpms, poolEnergies) {
  const med = median(poolBpms);
  const lo = percentile(poolBpms, 0.1);
  const hi = percentile(poolBpms, 0.9);
...
function transitionNote(prev, cur) {
  ...
  else how = 'two steps on the wheel — cut on the drop rather than blend';
...
function planSet(tracks, options = {}) {
  ...
  const usable = tracks.filter((t) => t && t.bpm > 0 && t.durationSec >= 90);
  if (usable.length === 0) return { tracks: [], totalSec: 0, medianBpm: null };

  const centre = median(usable.map((t) => t.bpm));
  ...
  const arc = buildArc(targetSec, pool.map((p) => p.bpm), pool.filter((p) => p.energy != null).map((p) => p.energy));

  function stepCost(prev, cur, t) {
    let c = 0;
    if (prev) {
      const cd = camelotDistance(prev.camelot, cur.camelot);
      if (cd >= 6) return Infinity;
      c += 3 * cd + (cd >= 2.5 ? 6 : 0);
      const bd = (Math.abs(cur.bpm - prev.bpm) / prev.bpm) * 100;
      if (bd > 3.5) return Infinity;
