...
// opening for however much of the clip is left — see realtimeBridge.js's
// intro-mixing code, and MUSIC_LOVERS_CALL_INTRO_ENABLED there for how to
// turn this off again). Reuses buildRingtoneClip/buildGeneratedRingtoneClip's
// own cache and source-resolution (real Spotify preview when available,
// else the ElevenLabs genre-matched fallback) so this never re-fetches or
...
//
// Sept 2026 update, Henry: bumped from 5 to 7 seconds total ("make it...
// last 7 sec in total"), then to 9 ("add 2sec more for sound background") —
// still comfortably inside CLIP_DURATION_SECONDS's 10s source above (leaves
// a full 1s of margin for the 0.4s fade-out below), so no change needed
// there.
const introClipCache = new Map();
const CALL_INTRO_SECONDS = 9;
...
async function buildCallIntroClip(trackId, previewUrl, genre, sampleRate = 16000) {
  // CALL_INTRO_SECONDS folded into the key so a clip cached under a
  // previous value of that constant (e.g. before this file's own Sept
  // 2026 5s -> 7s bump) never gets served back at the wrong length after a
  // deploy — see the CALL_INTRO_SECONDS comment above.
  const cacheKey = `${trackId}:${sampleRate}:${CALL_INTRO_SECONDS}`;
