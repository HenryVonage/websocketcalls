...
async function handleEvents(req, res) {
  ...
  try {
    ...
    if (body.status) {
      logEvent('call', `Call event: ${body.status}${body.from ? ` (${redactPhone(body.from)})` : ''}`, body.from);
    }
        ...
        if (context?.demo === DEMOS.TICKETING && context?.channel === 'whatsapp') {
          ...
          if (transcript) {
            ...
            if (demoChannel === 'rcs') {
              ...
              logEvent('outbound', `Sent RCS call recap (PDF link) to ${redactPhone(phone)}`, phone);
              ...
              if (booking?.wantsViewing) {
                ...
                logEvent('outbound', `Sent RCS appointment confirmation to ${redactPhone(phone)} (booking intent detected)`, phone);
              }
            } else {
              await sendVonageMessage({
              ...
              logEvent('outbound', `Sent henry_callrecap (PDF) to ${redactPhone(phone)}`, phone);
              ...
              logEvent('outbound', `Sent henry_form2 survey (Flow) to ${redactPhone(phone)}`, phone);
              ...
              if (booking?.wantsViewing) {
                ...
                logEvent('outbound', `Sent henryappointment to ${redactPhone(phone)} (booking intent detected)`, phone);
              }
            }
          } else {
            logEvent('call', `No transcript available for call with ${redactPhone(phone)} — recap skipped`, phone);
          }
        } else {
...
async function sendTicketingCallFollowUp(phone, context) {
  if (!context?.elevenConversationId) {
    logEvent('call', `No transcript available for Ticketing call with ${redactPhone(phone)} — follow-up skipped`, phone);
    return;
  }
  const transcript = await fetchTranscriptWithRetry(context.elevenConversationId);
  if (!transcript) {
  ...
  logEvent('outbound', `Sent Ticketing call follow-up to ${redactPhone(phone)}`, phone);
}
...
async function sendTicketingWhatsappCallFollowUp(phone, context) {
  if (!context?.elevenConversationId) {
    logEvent('call', `No transcript available for Ticketing WhatsApp call with ${redactPhone(phone)} — follow-up skipped`, phone);
    return;
  }
  const transcript = await fetchTranscriptWithRetry(context.elevenConversationId);
  if (!transcript) {
  ...
  logEvent('outbound', `Sent Ticketing WhatsApp call follow-up to ${redactPhone(phone)}`, phone);
...
async function sendMusicLoversCallFollowUp(phone, context, conversationUuid) {
  if (!context?.elevenConversationId) {
    logEvent('call', `No transcript available for Music Lovers call with ${redactPhone(phone)} — follow-up skipped`, phone);
    return;
  }
  const transcript = await fetchTranscriptWithRetry(context.elevenConversationId);
  if (!transcript) {
  ...
  if (ticketIntent?.wantsTickets && artist && venue) {
    ...
    // RCS counterpart to the henry_ticketing2 WhatsApp template below —
    // Henry's Sept 2026 report: a caller who rang the Music Lovers RCS
    // agent's shared PSTN number (config.RCS_PSTN_NUMBER) still got their
    // booking confirmation over WhatsApp, because this whole function
    // unconditionally sent WhatsApp templates regardless of which channel
    // handleAnswer's detectChannel actually tagged the call with (stashed
    // on context.channel — see handleEvents above, and the equivalent
    // rcs/whatsapp split the generic Real Estate branch already does for
    // its own call recap). RCS has no template-approval system, so this is
    // a direct card send — same "no henry_* template needed" pattern
    // already used by the Real Estate/Ticketing RCS branches — linking to
    // the same dynamically-generated ticket PDF handleMyTicketsButtonTap
    // serves for WhatsApp, rather than replicating the WhatsApp-only
    // ML_MY_TICKETS/ML_OTHER_QUESTION quick-reply round-trip.
    if (context?.channel === 'rcs') {
      const base = process.env.PUBLIC_BASE_URL || 'https://websocketcalls.onrender.com';
      const ticketUrl = `${base}/music-lovers/ticket/${ticketKey}.pdf`;
      await sendVonageMessage({
        from: config.RCS_AGENT_ID,
        to: phone,
        channel: 'rcs',
        message_type: 'text',
        text: `🎟️ You're booked in, ${ticketName}! Order ${orderNumber} for ${artist} at ${venue}.`,
        suggestions: [
          { type: 'open_url', text: 'View ticket', postback_data: 'view_ticket', url: ticketUrl, description: `Ticket for ${artist} at ${venue}` },
        ],
      });
      logEvent('outbound', `Sent RCS ticket confirmation (order ${orderNumber}, ${artist}) to ${redactPhone(phone)}`, phone);
      return;
    }

    // Per Henry's request: the confirmation header should show the actual
    // artist's own photo rather than the generic Music Lovers hero shot.
    // Best-effort — falls back to the static hero image on any failure
    // (network error, or an artist name Spotify's search can't match),
    // same fallback pattern musicLoversFlow.js already uses for the
    // matched-track album art.
    let headerImageUrl = musicConfig.GENRE_PROMPT_HEADER_IMAGE_URL;
    try {
      const artistImageUrl = await getArtistImageUrl(artist);
      if (artistImageUrl) headerImageUrl = artistImageUrl;
    } catch (err) {
      console.error('Music Lovers artist-image lookup failed (using hero image instead):', err.message);
    }
    ...
    await sendVonageMessage({
      ...
      custom: {
        ...
        template: {
          ...
          language: { policy: 'deterministic', code: 'en' },
          components: [
            // Header image: the artist's own photo (looked up above via
            // Spotify), falling back to the static Music Lovers hero image
            // (musicLoversConfig.js's GENRE_PROMPT_HEADER_IMAGE_URL) if the
    ...
    logEvent('outbound', `Sent henry_ticketing2 (order ${orderNumber}, ${artist}) to ${redactPhone(phone)}`, phone);
    return;
  }
  ...
  // Same RCS/WhatsApp split as the ticket-confirmation branch above, for
  // the plain-recap case (no ticket intent, or no concert on file yet).
  if (context?.channel === 'rcs') {
    await sendVonageMessage({
      from: config.RCS_AGENT_ID,
      to: phone,
      channel: 'rcs',
      message_type: 'text',
      text: `Hi ${callerName}, thanks for calling Music Lovers! Tap below for your call recap.`,
      suggestions: [
        { type: 'open_url', text: 'View recap', postback_data: 'view_recap', url: documentUrl, description: 'Call recap PDF' },
      ],
    });
    logEvent('outbound', `Sent RCS call recap (PDF link) to ${redactPhone(phone)} (Music Lovers, no ticket intent)`, phone);
    return;
  }
  ...
  logEvent('outbound', `Sent henry_callrecap (PDF) to ${redactPhone(phone)} (Music Lovers, no ticket intent)`, phone);
}
...
async function sendFeedbackThankYou(phone, demoChannel = 'whatsapp') {
  ...
  logEvent('outbound', `Sent feedback thank-you to ${redactPhone(phone)} (${demoChannel})`, phone);
}

module.exports = { handleAnswer, handleEvents, buildMusicLoversDynamicVariables };
