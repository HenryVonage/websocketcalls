...
async function sendFollowUp(phone, context) {
  ...
  await sendVonageMessage({
    ...
    custom: {
      ...
      template: {
        ...
        language: { policy: 'deterministic', code: 'en' },
        components: [
          // Approved template has an IMAGE header — see the matching fix
          // in whatsappFlow.js's [VIEWING_INTEREST] step for details.
  ...
  logEvent('outbound', `Sent henry_confirmationviewing follow-up to ${redactPhone(phone)}`);
}
