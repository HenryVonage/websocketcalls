...
async function sendMusicLoversCallFollowUp(phone, context, conversationUuid) {
  ...
  if (ticketIntent?.wantsTickets && artist && venue) {
    ...
    if (context?.channel === 'rcs') {
      ...
      logEvent('outbound', `Sent RCS ticket confirmation (order ${orderNumber}, ${artist}) to ${redactPhone(phone)}`);
      return;
    }
  ...
  if (context?.channel === 'rcs') {
    ...
    logEvent('outbound', `Sent RCS call recap (PDF link) to ${redactPhone(phone)} (Music Lovers, no ticket intent)`);
    return;
  }
