...
const { getCallContext, setCallContext, setCallSummaryText, getCallerName, getActiveDemo } = require('./store');
...
// used for callerName.
//
// The Ticketing WhatsApp demo has no such ambiguity — it dials its OWN
// dedicated WABA number (config.TICKETING.WHATSAPP.FROM_WHATSAPP), never
// shared with Real Estate, so a call landing there is unambiguously
// Ticketing regardless of any RCS-side state for that phone number.
//
// Falls back to 'real-estate' for a cold call on the RCS PSTN number with
// no prior RCS message, and for anything on the original Real Estate
// WhatsApp-calling number.
function detectDemo(toNumber, fromNumber) {
  if (normalizeNumber(toNumber) === normalizeNumber(config.TICKETING.WHATSAPP.FROM_WHATSAPP)) return DEMOS.TICKETING;
  if (detectChannel(toNumber) !== 'rcs') return DEMOS.REAL_ESTATE;
