...
//     stash a fake ticket for the (not yet wired) "My tickets over
//     WhatsApp" button.
...
//
// KNOWN GAP, deliberately left for a follow-up once confirmed: the
// henry_ticketing2 send below only sets the header + 4 body params exactly
// as businessConfig.js documents that template's approved shape (image
// header + name/order-number/event-name x2) — it does NOT include a button
// component, because the real "My tickets over WhatsApp" button's sub_type
// (quick_reply vs. flow vs. a plain url button) has never been confirmed —
// two image attachments of the template's design failed to come through in
// chat. Use the enhanced /admin/whatsapp-templates?waba=<id>&name=henry_ticketing2
// diagnostic route (server.js) to get the real shape from Vonage/Meta
// directly, then wire the button here and the tap-handler in
// musicLoversFlow.js's handleMusicLoversInbound. If Vonage rejects this send
// outright for a missing button component, that confirms the button is
// required and unblocks the same fix.
async function sendMusicLoversCallFollowUp(phone, context, conversationUuid) {
  ...
  if (ticketIntent?.wantsTickets && artist && venue) {
    ...
    // henry_ticketing2 (per businessConfig.js's documented shape: image
    // header + 4 body vars — name, order number, event name x2). Henry's
    // own instruction was explicit that BOTH {{3}} and {{4}} should carry
    // the venue name (replacing the sample's "Rolex Monte Carlo Masters
    // 2025" in each), not two different values — followed literally here
    // even though that reads as a duplicate.
    await sendVonageMessage({
      from: config.FROM_WHATSAPP,
      to: phone,
      channel: 'whatsapp',
