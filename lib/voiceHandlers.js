...
function handleAnswer(req, res) {
  ...
  setCallContext(q.conversation_uuid, {
    ...
    // landing page's pre-filled greeting (see whatsappFlow.js) — lets the
    // henry_callrecap template use their real first name instead of the
    // generic "there" fallback.
    callerName: getCallerName(q.from),
  });
  logEvent('call', `Inbound call answered from ${redactPhone(q.from)}`);
    ...
    if (body.status === 'completed') {
      ...
      if (phone) {
        if (context?.context === 'feedback_call') {
          await sendFeedbackThankYou(phone);
                  ...
                  template: {
                    ...
                    components: [
                      { type: 'header', parameters: [{ type: 'location', location: config.VONAGE_OFFICE }] },
                      {
                        type: 'body',
                        parameters: [
                          { type: 'text', text: context?.callerName || booking.name || 'there' },
                          { type: 'text', text: booking.propertyName || "Regent's Park" },
                          { type: 'text', text: booking.appointmentTime || 'a time our team will confirm with you' },
...
async function sendFeedbackThankYou(phone) {
  await sendVonageMessage({
    from: config.FROM_WHATSAPP,
    to: phone,
    channel: 'whatsapp',
  ...
  logEvent('outbound', `Sent feedback thank-you to ${redactPhone(phone)}`);
}
