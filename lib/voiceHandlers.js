...
// All three demos share one Vonage Application, Answer URL, and Event URL —
...
// tag; everything else (the original WhatsApp-calling number) keeps the
// existing 'whatsapp' behavior. Used below to decide where the post-call
// recap/appointment gets sent.
function detectChannel(toNumber) {
  return normalizeNumber(toNumber) === normalizeNumber(config.RCS_PSTN_NUMBER) ? 'rcs' : 'whatsapp';
}

// Ticketing's "Contact us" escalation dials the SAME PSTN number as the
...
// used for callerName. Falls back to 'real-estate' for a cold call with no
// prior RCS message, and for anything on the WhatsApp-calling number (which
// only ever serves Real Estate).
...
async function handleEvents(req, res) {
  ...
  try {
    ...
    if (body.status === 'completed') {
      ...
      if (phone) {
        if (context?.demo === DEMOS.TICKETING) {
