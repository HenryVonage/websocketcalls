...
function waitForDelivery(messageUuid, timeoutMs = 15000) {
  ...
  return new Promise((resolve) => {
    ...
    const done = () => {
      if (resolved) return;
      resolved = true;
      // notifyDelivered() already removes the entry on the happy path
      // (the DLR arrived first) — this only needs to clean up the
      // timeout path, where the entry would otherwise sit in `pending`
      // forever (no "delivered" DLR ever arrives, or it arrives as
      // "read"/late). Without this, every WhatsApp send that doesn't get
      // a clean, on-time DLR leaked one Map entry for the life of the
      // process.
      const list = pending.get(messageUuid);
      if (list) {
        const next = list.filter((fn) => fn !== done);
        if (next.length) pending.set(messageUuid, next);
        else pending.delete(messageUuid);
      }
      resolve();
    };
    const list = pending.get(messageUuid) || [];
