...
// given agent — Henry's Sept 2026 question: addRcsTestDevice above only
// tells us Vonage ACCEPTED a registration request, not that the number is
// really usable yet. Per Vonage's docs (developer.vonage.com/en/messages/
// guides/rcs/rcs-device-capability-check), a device only shows as RCS-
// capable to a *test* agent once it's allow-listed — and allow-listing a
// number also triggers Google's own "make me a tester" SMS invite, which
// the number's owner has to accept before the agent can message them (see
// demo.html's registerTester() comment — confirmed the hard way once
// already). So `rcs_supported: false` here should mean "not ready yet",
// covering both "never registered" and "registered but invite not
// accepted" — but Vonage's public docs don't explicitly say the check
// distinguishes those two cases, so treat this as unconfirmed until tested
// against a real pending-invite number (e.g. re-run against the friend's
// number from the Sept 15 test) and adjust if the response shape says
// otherwise. Unlike addRcsTestDevice/listRcsAgents (account-level Basic
// auth), this is an Application-scoped endpoint per Vonage's docs, so it
// uses the same JWT bearer auth as sendVonageMessage/createVonageCall.
// `senderId` is the human-readable sender_id (RCS_AGENT_SENDER_ID, e.g.
// "henry_rcs_demo3") per Vonage's own example — NOT the internal
// RCS_AGENT_ID_CM uuid the test-devices endpoint needs; if this 404s,
