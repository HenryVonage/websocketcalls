...
// linkResolvers.js's resolveConcertDetails. Used only by pushPretendConcert
// below (Henry's Sept 2026 "pretend the user could be interested" request),
// once the negative-feedback loop's 2-song cap is reached — a real lookup
// would need a real artist name, and most of this demo's catalog is still
// TRACK_CATALOG's placeholder data. Venue is picked deterministically from
// the artist's name (musicConfig.PRETEND_CONCERT_VENUES) so re-sends for the
// same artist land on the same pretend venue/date rather than a fresh one
// each time; date is formatted the same way resolveConcertDetails formats a
// real one, ~6 weeks out.
...
// "TBD" and every fallback the same "TBD").
