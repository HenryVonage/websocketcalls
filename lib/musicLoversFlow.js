...
// Henry's recent listening matched this bucket). youtubeVideoId comes from
// the same refresh — server.js looks it up dynamically via
// lib/youtubeApi.js's searchVideoId() for whichever track ends up on top of
// each genre's bucket, since Spotify's API has no YouTube video mapping of
// its own — and only falls back to the static entry's placeholder if that
// lookup was never run or came back empty for this track.
