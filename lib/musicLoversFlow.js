...
const mixVibes = require('./mixVibes');
...
async function pushPretendConcert(phone, userName, state) {
  ...
  logEvent('outbound', `Pushed a pretend concert for ${artist} to ${redactPhone(phone)} (2-song negative-feedback cap reached, no concert interest shown)`, phone);
}
...
async function sendAlternateSong(phone, userName, state) {
  ...
  if (!alt) {
    logEvent('outbound', `Music Lovers: no alternate track available for ${redactPhone(phone)} — skipped negative-feedback resend`, phone);
    return;
  }
...
async function handleNegativeFeedback(phone, userName, state) {
  ...
  if (songsSent >= 2) {
    logEvent('inbound', `Music Lovers: negative feedback from ${redactPhone(phone)} after the 2-song cap — no further resend`, phone);
    return;
  }
...
async function handleSpotifyConnected(phone, userName) {
  try {
    ...
    setMusicLoversState(phone, {
      ...
      ...track,
      viaSpotify: true,
      // Clears any concert info left over from a PREVIOUS song/artist on
      // this phone (e.g. an earlier demo run) — otherwise
      // buildMusicLoversDynamicVariables (voiceHandlers.js) would still
      // prefer that stale concertArtist over this new match if the caller
      // rings in without ever asking about concerts for it. Same fix as
      // sendAlternateSong's own concertArtist/concertDetails clear below.
      concertArtist: undefined,
      concertDetails: undefined,
    });
    logEvent('outbound', `Matched ${redactPhone(phone)} to genre "${genre}" via Spotify connect`, phone);
  } catch (err) {
    console.error('Music Lovers Spotify-connected handling error:', err.message);
    logEvent('outbound', `Spotify connect matching failed for ${redactPhone(phone)}, falling back to genre prompt`, phone);
...
async function handleGenrePickerFlowCompletion(body) {
  ...
  logEvent('inbound', `Music Lovers genre-picker Flow response from ${redactPhone(phone)}: ${JSON.stringify(answers)}`, phone);
  ...
  if (genres.length === 0) {
    ...
    logEvent('outbound', `Genre-picker Flow response from ${redactPhone(phone)} didn't match a known genre`, phone);
    return;
  }
  ...
  await sendMatchedTrack(phone, userName, track);
  // concertArtist/concertDetails cleared here too — same staleness fix as
  // handleSpotifyConnected/sendAlternateSong above (see their comments):
  // without this, a listener who re-picks a genre without a fresh "Hi"
  // reset keeps whatever concert info an EARLIER song on this phone left
  // behind, and a voice call in would talk about that old artist/concert
  // instead of the one just matched.
  setMusicLoversState(phone, { stage: 'awaiting_confirmation', genre, ...track, concertArtist: undefined, concertDetails: undefined });
  logEvent(
    'outbound',
...
async function handleExploreMoreFlowCompletion(body, state) {
  ...
  logEvent('inbound', `Music Lovers explore-more Flow response from ${redactPhone(phone)}: ${JSON.stringify(answers)}`, phone);
  ...
  if (likesStuckInHead && state.spotifyTrackId) {
    ...
    logEvent('outbound', `Sent ringtone clip to ${redactPhone(phone)} (via explore-more Flow)`, phone);
  ...
  if (wantsLyrics && artist && title) {
    ...
    logEvent('outbound', `Sent lyrics link to ${redactPhone(phone)}`, phone);
  }
  ...
  if (wantsBio && artist) {
    ...
    logEvent('outbound', `Sent artist bio link to ${redactPhone(phone)}`, phone);
  }
  ...
  if (wantsConcerts && artist) {
    ...
    if (details) {
      ...
      setMusicLoversState(phone, { concertArtist: artist, concertDetails: details });
      logEvent('outbound', `Sent henry_ticketingconcert template to ${redactPhone(phone)}`, phone);
    } else {
      ...
      logEvent('outbound', `Sent concerts link to ${redactPhone(phone)} (no full event details available)`, phone);
    }
  }
  ...
  if (wantsMoreSongs && artist) {
    ...
    logEvent('outbound', `Sent more-songs link to ${redactPhone(phone)}`, phone);
  }
...
async function handleMyTicketsButtonTap(body) {
  ...
  logEvent('inbound', `"My tickets" tapped by ${redactPhone(phone)}`, phone);
  ...
  if (!ticket) {
    ...
    logEvent('outbound', `No ticket on file for ${redactPhone(phone)} — sent fallback text instead`, phone);
    return;
  }
  ...
  logEvent('outbound', `Sent ticket PDF (order ${ticket.orderNumber}) to ${redactPhone(phone)}`, phone);
}
...
async function triggerOtherQuestionCall(phone) {
  ...
  logEvent('call', `Placed "other question" outbound call to ${redactPhone(phone)}`, phone);
}
...
async function handleOtherQuestionButtonTap(body) {
  const phone = body.from ?? '';
  const userName = getCallerName(phone) || body.profile?.name || 'there';
  logEvent('inbound', `"Other question" tapped by ${redactPhone(phone)}`, phone);
  ...
  if (perm?.requestedAt && now - perm.requestedAt < 24 * 60 * 60 * 1000) {
    ...
    logEvent('outbound', `Call-permission request already pending for ${redactPhone(phone)} — sent reminder instead of re-sending`, phone);
    return;
  }
  ...
  logEvent('outbound', `Sent henry_callpermission2 call-permission request to ${redactPhone(phone)} — placing the call in 9s`, phone);
...
// --- "Build me a mix" over WhatsApp (Sept 2026) ---
// The personal DJ mix (lib/musicLoversMix.js) used to be reachable only
// from demo.html; Henry's first live test was to simply ask for one in
// the chat — and got silence, because a listener whose journey is already
// `done` had no free-text handling at all. So at ANY stage a message
// that mentions a mix / DJ set kicks one off from Henry's library, with
// vibe and length read from the text ("a 30 min chilled mix", "workout
// mix for an hour") and the house-party hour as the default. Lazy require
// avoids a cycle: musicLoversMix pulls store + vonageApi, not this file,
// but keeping it lazy also keeps the ringtone path's startup untouched.
const MIX_REQUEST_RE = /\b(dj\s*set|mix(?:tape|es)?|set\s*list|playlist for (?:my|a) (?:party|workout|run))\b/i;
const VIBE_HINTS = [
  ['chilled', /\b(chill|chilled|relax|calm|sunset|lounge|downtempo|slow)\b/i],
  ['madness', /\b(madness|mad|hard|crazy|peak|rave|banger|bangers|techno|wild)\b/i],
  ['workout', /\b(workout|gym|run|running|training|cardio|hiit|sport)\b/i],
  ['motivational', /\b(motivat\w*|uplift\w*|inspir\w*|anthem|pump)\b/i],
  ['houseparty', /\b(party|house|dance|club|clubbing)\b/i],
];

function parseMixRequest(text) {
  const t = String(text || '');
  if (!MIX_REQUEST_RE.test(t)) return null;
  let vibe = mixVibes.DEFAULT_VIBE;
  for (const [key, re] of VIBE_HINTS) {
    if (re.test(t)) {
      vibe = key;
      break;
    }
  }
  let minutes = mixVibes.DEFAULT_DURATION_MIN;
  const m = t.match(/(\d{1,3})\s*(?:min|mins|mn|minutes?)\b/i) || t.match(/(\d(?:[.,]\d)?)\s*(?:h|hr|hrs|hours?)\b/i);
  if (m) {
    const n = Number(String(m[1]).replace(',', '.'));
    const asMin = /h/i.test(m[0].replace(m[1], '')) ? n * 60 : n;
    // Snap to the nearest supported length (30 / 60 / 90).
    minutes = mixVibes.DURATIONS_MIN.reduce((best, d) => (Math.abs(d - asMin) < Math.abs(best - asMin) ? d : best), mixVibes.DURATIONS_MIN[0]);
  } else if (/\b(an?|one)\s+hour\b/i.test(t)) minutes = 60;
  else if (/\bhalf\s+an?\s+hour\b/i.test(t)) minutes = 30;
  return { vibe, durationMin: minutes };
}
...
async function handleMusicLoversInbound(req, res) {
  ...
  // Tracked outside the try block so the catch below can still reach the
  // listener even if the crash happened before/while resolving it (e.g. a
  // malformed body) — see the catch's own comment for why this matters.
  let phoneForErrorFallback = '';

  try {
    const body = req.body || {};
    phoneForErrorFallback = body.from ?? '';
    ...
    logEvent('inbound', `Music Lovers message from ${redactPhone(phone)}: "${messageText}"`, phone);
    ...
    if (isResetGreetingFor(DEMOS.MUSIC_LOVERS, messageText)) {
      ...
      logEvent('inbound', `Reset phrase detected — cleared Music Lovers state for ${redactPhone(phone)}`, phone);
    }

    const state = getMusicLoversState(phone);

    // "Can you build me a 30 min workout mix?" — works at every stage,
    // including `done`, and doesn't move the genre/ringtone journey on.
    const mixRequest = parseMixRequest(messageText);
    if (mixRequest) {
      const { buildAndSendMix } = require('./musicLoversMix');
      logEvent('inbound', `Mix requested over WhatsApp by ${redactPhone(phone)} (${mixRequest.vibe}, ${mixRequest.durationMin} min)`, phone);
      buildAndSendMix(phone, userName, { ...mixRequest, source: 'henry' }).catch((err) => {
        console.error('buildAndSendMix (WhatsApp keyword) failed:', err);
      });
      return;
    }
    ...
    if (!state.stage) {
      await sendGenrePrompt(phone, userName);
      setMusicLoversState(phone, { stage: 'awaiting_genre' });
      logEvent('outbound', `Sent genre prompt to ${redactPhone(phone)}`, phone);
      return;
    }
    ...
    if (state.stage === 'awaiting_genre') {
      ...
      await sendMatchedTrack(phone, userName, track);
      // Same concertArtist/concertDetails staleness fix as the genre-picker
      // Flow branch above — see that call's comment.
      setMusicLoversState(phone, { stage: 'awaiting_confirmation', genre, ...track, concertArtist: undefined, concertDetails: undefined });
      logEvent(
        'outbound',
    ...
    if (state.stage === 'awaiting_confirmation') {
      ...
      await sendRingtone(phone, state);
      setMusicLoversState(phone, { stage: 'done' });
      logEvent('outbound', `Sent ringtone clip to ${redactPhone(phone)}`, phone);
      return;
    }
    ...
    // Henry's Sept 2026 report: picking "Classic"/"Movie soundtrack" once
    // appeared to leave the listener with no reply at all — whatever the
    // original trigger was, ANY unhandled error in the try block above has
    // always had this same silent failure mode, since the 200 ack above
    // only tells Vonage the webhook was received, not that a reply went
    // out. A demo visitor has no way to see the server console, so from
    // their side "an error was logged" and "the bot just ignored me" are
    // indistinguishable — this best-effort fallback closes that gap by
    // making sure *something* always comes back, even when it's just an
    // apology. Deliberately its own try/catch: this must never let a
    // second failure (e.g. sendVonageMessage itself erroring) escape and
    // override the real error already logged above.
    if (phoneForErrorFallback) {
      try {
        await sendVonageMessage({
          from: musicConfig.FROM_WHATSAPP,
          to: phoneForErrorFallback,
          channel: 'whatsapp',
          message_type: 'text',
          text: "Sorry, something went wrong matching your genre — mind trying again? Reply with one of: " + musicConfig.GENRES.join(', ') + '.',
        });
      } catch (fallbackErr) {
        console.error('Music Lovers error-fallback message also failed to send:', fallbackErr);
      }
    }
  }
}

module.exports = { handleMusicLoversInbound, matchGenre, handleSpotifyConnected, parseMixRequest };
