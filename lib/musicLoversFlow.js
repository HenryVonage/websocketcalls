...
// nameCapture.js's own isResetGreeting is hardcoded to the Real Estate
// demo's greeting text ("welcome" + "real estate demo") — it was being
// imported and used below unchanged, so a re-sent Music Lovers greeting
// ("Hi, Im Henry — Welcome to Music Lovers demo!") never matched and never
// reset an in-progress journey. Same bug shape ticketingWhatsappFlow.js
// already avoids by defining its own local isResetGreeting rather than
// reusing nameCapture.js's — doing the same here now.
function isResetGreeting(text) {
  const t = (text || '').toLowerCase();
  return t.includes('welcome') && t.includes('music lovers demo');
}
...
async function sendGenrePrompt(phone, userName) {
  ...
  // love!" — exactly 2 body params, no header. Sending components: []
  // (the original placeholder before this was confirmed) got this
  // rejected outright by Vonage: error 1022 "number of localizable_params
  // (0) does not match the expected number of params (2)" — the send
  // never reached the recipient at all, not even silently.
  //
