...
async function handleExploreMoreFlowCompletion(body, state) {
  ...
  // form, in addition to being what Henry asked for. Search-results URLs
  // rather than a guessed direct page URL (e.g. wikipedia.org/wiki/<Artist>)
  // since a slug guess can easily 404 for a real artist name.
  if (wantsLyrics && artist && title) {
    const url = `https://genius.com/search?q=${encodeURIComponent(`${artist} ${title}`)}`;
    await sendVonageMessage({
      from: musicConfig.FROM_WHATSAPP,
      to: phone,
      channel: 'whatsapp',
  ...
  if (wantsBio && artist) {
    const url = `https://en.wikipedia.org/w/index.php?search=${encodeURIComponent(artist)}`;
    await sendVonageMessage({
      from: musicConfig.FROM_WHATSAPP,
      to: phone,
      channel: 'whatsapp',
  ...
  if (wantsConcerts && artist) {
    const url = `https://www.songkick.com/search?query=${encodeURIComponent(artist)}&type=artists`;
    await sendVonageMessage({
      from: musicConfig.FROM_WHATSAPP,
      to: phone,
      channel: 'whatsapp',
