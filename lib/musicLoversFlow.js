...
// Matches free text (or a Flow answer's flattened text) against the genre
// catalog — case-insensitive, via musicConfig.GENRE_SYNONYMS (see that
// file's comments for the full list of real Flow checkbox labels each
...
//
// Returns every genre that matches, not just the first — the Flow's genre
// question is "Choose all that apply" (multi-select), and Henry's Sept
// 2026 request needs the full list so pickTrackForGenres() below can fall
// back to a different one of the listener's OWN picks when the
// highest-priority match has no cached Top Track. matchGenre() (kept for
// every other, single-genre call site: handleSpotifyConnected's bucket
// lookup, etc.) is just this list's first entry.
...
// Henry's recent listening matched this bucket).
//
// youtubeVideoId: the refresh job (server.js) already resolves this
// dynamically via lib/youtubeApi.js's searchVideoId(), searching for
// whichever track Spotify just picked, and stores the result on the cached
// `top` track — so it changes in lockstep with the Spotify pick, same as
// title/artist/spotifyTrackId, rather than needing a manual re-pick. This
// used to only copy spotifyTrackId/title/artist and silently drop the
// resolved youtubeVideoId, always falling through to the static catalog's
// placeholder even on a successful lookup — fixed below. Only falls back to
// the static entry's youtubeVideoId when the lookup genuinely failed or was
// skipped (YOUTUBE_API_KEY not set, search error, no results — see
// searchVideoId's own doc comment) and left it unset on `top`.
...
// genre+track to actually send — Henry's Sept 2026 rules for the Flow's
// "Choose all that apply" genre question:
...
// Places the actual outbound WhatsApp call, triggered from
// handlePendingCallPermissionFollowUp below. Same connect->websocket path
// every other call in this codebase uses (buildAnswerNcco/realtimeBridge.js)
// — context 'music_lovers_other_question' starts with 'music_lovers', so
// realtimeBridge.js's existing isMusicLovers check already routes it to
// ELEVENLABS_MUSIC_AGENT_ID with no further changes needed there.
//
...
// Manager) and, once granted, place an outbound call so the caller can ask
// their question live instead of typing it.
//
...
// call permission settings." So this only ever sends the request; the call
// itself is placed later, from the caller's next message/tap in this chat
// (handlePendingCallPermissionFollowUp below) — Henry's own choice among
// three options discussed, trading a small chance of firing before they've
// actually responded to the prompt for not needing any extra wait/trigger.
async function handleOtherQuestionButtonTap(body) {
  ...
  // get silently dropped — nudge the caller back to the one already
  // sitting in their chat instead of re-sending. (No way to tell whether
  // they've already accepted it and just haven't sent a follow-up message
  // yet — see this function's own comment above on why no such signal
  // exists.)
  if (perm?.requestedAt && now - perm.requestedAt < 24 * 60 * 60 * 1000) {
    await sendVonageMessage({
      ...
      channel: 'whatsapp',
      message_type: 'text',
      text: `Hi ${userName}, I already sent you a request to enable WhatsApp calling — just tap Accept on that one, then send me anything here and I'll call you right after.`,
    });
  ...
  logEvent('outbound', `Sent henry_callpermission2 call-permission request to ${redactPhone(phone)}`);
}

// Fires the outbound call on the caller's first message/tap after a
// call-permission request went out (see handleOtherQuestionButtonTap's
// comment for why there's no accept/decline signal to wait for instead).
// `attempted` guards against a burst of quick follow-up messages placing
// more than one call for the same request.
async function handlePendingCallPermissionFollowUp(body) {
  const phone = body.from ?? '';
  setMusicLoversState(phone, {
    callPermission: { ...getMusicLoversState(phone).callPermission, attempted: true, attemptedAt: Date.now() },
  });
  logEvent(
    'call',
    `Caller's next message after the call-permission request — attempting outbound call to ${redactPhone(phone)} now (no accept/decline signal to wait for)`
  );
...
async function handleMusicLoversInbound(req, res) {
  ...
  try {
    ...
    // A call-permission request is outstanding for this phone and hasn't
    // been followed up on yet — treat this inbound event (whatever it is)
    // as "they're back" and place the outbound call now, per
    // handleOtherQuestionButtonTap's comment on why there's no
    // accept/decline webhook to wait for instead. Skipped when this event
    // IS another "OTHER QUESTION" tap — that has its own handling for a
    // still-outstanding request (a reminder text, not a call attempt).
    const isOtherQuestionTap = body.message_type === 'button' && body.button?.payload === 'ML_OTHER_QUESTION';
    const pendingPermission = getMusicLoversState(body.from ?? '').callPermission;
    if (pendingPermission?.requestedAt && !pendingPermission.attempted && !isOtherQuestionTap) {
      await handlePendingCallPermissionFollowUp(body);
      return;
    }
    ...
    // and places an outbound call on the caller's next message — see
