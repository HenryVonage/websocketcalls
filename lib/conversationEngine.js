...
STEP 3B — message_type=button AND message='share_location': this is only the acknowledgment that the user tapped the "Share location" chip and their device's location picker just opened — it is NOT their actual location yet, and does not satisfy STEP 4. Take no action.
Output exactly: [IGNORE]

STEP 4 — message_type is exactly 'location' (the user's actual GPS coordinates arrived, not merely a tap on the share-location chip):
...
Output exactly: [TIMESLOT_LIST: <first_name>]

STEP 6B — message_type=reply AND the previous bot message offered a choice of viewing timeslots:
Extract user first name from conversation history (default: Client). The chosen timeslot is the current message's text, verbatim.
Output exactly: [VIDEO_TOUR: <first_name>|<chosen_timeslot>]
...
- [IGNORE] (STEP 3B) is a valid output — it means take no action, not an error
...
async function decideMarker(fields) {
  ...
  const history = getConversation(fields.clientPhone, 'real-estate');
  ...
  pushTurn(fields.clientPhone, 'user', userMessage, 'real-estate');
  pushTurn(fields.clientPhone, 'assistant', output, 'real-estate');
  return output;
}
