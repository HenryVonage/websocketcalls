...
// henry_musicsharing3 template's "Play it on Spotify" button and to look up
...
// Submitted to WhatsApp Manager Sept 2026 (category "Calling permissions
// request") so henry_ticketing2's "OTHER QUESTION" button can request
// WhatsApp-calling permission before placing an outbound call — see
// musicLoversFlow.js's handleOtherQuestionButtonTap. Meta requires this
// component type's template to be body-text-only (no header/footer/custom
// buttons), so unlike every other template in this file there's no header
// image to configure here.
const CALL_PERMISSION_TEMPLATE_NAME = 'henry_callpermission2';

// Sept 2026, Henry: "push another song suggestion from another artist" when
// the listener says they're not into the matched track — a loose substring
// match, same reasoning as RINGTONE_CONFIRM_RE below, so small wording
// variations still catch it. Covers both "it's okay but nothing special"
// (via "nothing special") and "I hate it". Checked against both a
// free-typed reply during awaiting_confirmation and (after Flow Builder's
// own underscore normalization) an "Explore more & give feedback" Flow
// answer — see musicLoversFlow.js's handleNegativeFeedback. If/when Henry
// adds a rating question with option labels matching these exact phrases to
// that Flow in WhatsApp Manager, this same regex already covers it with no
// further code change.
const NEGATIVE_FEEDBACK_RE = /nothing special|i hate it/i;

// Fabricated venues — NOT a real Ticketmaster lookup — used only by
// musicLoversFlow.js's pushPretendConcert. Henry's Sept 2026 request: once
// the 2-song negative-feedback loop below caps out, push a concert
// suggestion for the 2nd song's artist regardless of whether a real one
// exists, since most of this demo's catalog (TRACK_CATALOG above) is still
// placeholder data a real Ticketmaster lookup would never match anyway.
// Generic, recognizable venues so the suggestion still reads as plausible
// on screen.
const PRETEND_CONCERT_VENUES = [
  'The O2 Arena, London',
...
module.exports = {
  ...
  CALL_PERMISSION_TEMPLATE_NAME,
  NEGATIVE_FEEDBACK_RE,
  PRETEND_CONCERT_VENUES,
  trackForGenre,
