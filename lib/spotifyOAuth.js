...
// Music Lovers "Connect your Spotify" step (frontend/demo.html's Music Lovers family card) —
...
// Extra scopes the "Personal DJ mix" option needs (lib/musicLoversMix.js):
// reading the listener's playlists and writing the finished mix back as a
// private playlist on their account. Only requested when the visitor
// ticked that option on demo.html, so the plain genre-match path keeps
// its narrower read-only consent screen.
const MIX_SCOPES = 'user-top-read playlist-read-private playlist-read-collaborative playlist-modify-private';
...
// `extra` carries per-connect options through the OAuth round trip (today
// just { wantMix: true } for the Personal DJ mix option) — it lives here
// rather than in the `state` param itself so nothing user-controlled ever
// gets round-tripped through Spotify's redirect.
function createPendingState(phone, name, extra = {}) {
  ...
  pendingState.set(token, { phone, name, ...extra, createdAt: now });
  return token;
}
...
function getAuthorizeUrl(state, { wantMix = false } = {}) {
  ...
  const params = new URLSearchParams({
    ...
    scope: wantMix ? MIX_SCOPES : SCOPES,
    state,
