...
// Matches the greeting frontend/demo.html's DEMOS entry builds for Music
// Lovers (see its inline script's greeting() function on that entry):
// "...Welcome to Music Lovers demo!" — same loose,
// content-based routing as Ticketing above, sharing this WABA number too
// (see lib/musicLoversConfig.js).
const MUSIC_LOVERS_KEYWORD_RE = /music lovers/i;

const DEMOS = { REAL_ESTATE: 'real-estate', TICKETING: 'ticketing', MUSIC_LOVERS: 'music-lovers' };
...
function detectDemoFromText(text) {
  ...
  if (TICKETING_KEYWORD_RE.test(t)) return DEMOS.TICKETING;
  if (MUSIC_LOVERS_KEYWORD_RE.test(t)) return DEMOS.MUSIC_LOVERS;
...
// Whether `text` looks like the QR-triggered "fresh start" greeting for
// `demo` specifically — built on the exact same regexes as
// detectDemoFromText/resolveDemo above, instead of each demo's flow file
// hand-rolling its own "welcome" + topic-keyword check. Three separate
// hand-rolled copies of this (nameCapture.js for Real Estate,
// ticketingWhatsappFlow.js for Ticketing WhatsApp, musicLoversFlow.js for
// Music Lovers) is exactly how a copy-pasted, never-updated check caused
// the Music Lovers greeting-reset bug (Sept 2026): resending that demo's
// own greeting didn't reset an in-progress journey because the local copy
// still only matched Real Estate's phrase. A greeting should only reset a
// demo's own conversation state when it actually names that demo — this
// is also why Ticketing's RCS path (processTicketingRcs, ticketingFlow.js)
// never reset at all: it had no reset check of any kind until this shared
// helper existed to wire in.
function isResetGreetingFor(demo, text) {
  return detectDemoFromText(text) === demo;
}

module.exports = { DEMOS, detectDemoFromText, resolveDemo, isResetGreetingFor };
