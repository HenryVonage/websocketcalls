...
async function getOwnerTracks(accessToken, { force = false } = {}) {
  ...
  libraryCache = { at: Date.now(), tracks: [...byId.values()], playlistNames: playlists.map((p) => p.name) };
  return libraryCache;
}

// ---------- pre-analysis job ----------
const job = { running: false, startedAt: null, finishedAt: null, total: 0, done: 0, analysed: 0, error: null };
...
async function preanalyse() {
  ...
  (async () => {
    try {
      ...
      const { tracks } = await getOwnerTracks(token, { force: true });
