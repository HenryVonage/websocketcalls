...
const { getSpotifyTokens, setSpotifyTokens, forgetMissingTrackFeatures } = require('./store');
...
// ---------- pre-analysis job ----------
const job = { running: false, startedAt: null, finishedAt: null, total: 0, done: 0, analysed: 0, withPreview: 0, forgotten: 0, error: null, playlists: [], skipped: 0 };

// options.retryMissing — forget cached "no preview" results for the
// library first, so tracks written off during an iTunes throttle get
// another go (slow: a full library at the paced rate is ~15–20 min).
async function preanalyse({ retryMissing = false } = {}) {
  if (job.running) return { ...job };
  Object.assign(job, { running: true, startedAt: Date.now(), finishedAt: null, total: 0, done: 0, analysed: 0, withPreview: 0, forgotten: 0, error: null });
  (async () => {
    try {
      ...
      job.skipped = skipped;
      if (retryMissing) job.forgotten = forgetMissingTrackFeatures(tracks.map((t) => t.id));
      ...
      job.withPreview = out.filter((t) => t.previewUrl).length;
    } catch (err) {
