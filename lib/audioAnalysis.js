...
//
//   tempo  — spectral-flux onset envelope, autocorrelated over the 60–200
//            BPM lag range, scored against a log-normal prior centred on
//            ~120 BPM to break octave ties (the classic 65 vs 130 problem).
...
function analyseFrames(pcm) {
  ...
  // Precompute bin -> pitch-class mapping for the chroma. Range 120 Hz –
  // 2.5 kHz: at 11 025 Hz / 1024-point frames a bin is ~10.8 Hz wide, so
  // below ~120 Hz one bin spans a whole semitone or more AND that's where
  // the kick drum lives — with a 65 Hz floor every four-to-the-floor track
  // came out "D minor" because the kick's body landed in the lowest
  // included bin (75 Hz ≈ D). Above 2.5 kHz overtones muddy the profile.
  // Only bins that sit within ±35 cents of a semitone centre count, so a
  // wide low bin can't smear across two pitch classes.
  const binPc = new Int8Array(FRAME / 2).fill(-1);
  for (let b = 1; b < FRAME / 2; b++) {
    const hz = (b * SAMPLE_RATE) / FRAME;
    if (hz < 120 || hz > 2500) continue;
  ...
  for (let f = 0; f < nFrames; f++) {
    ...
    for (let b = 1; b < FRAME / 2; b++) {
      ...
      const pc = binPc[b];
      if (pc >= 0) chroma[pc] += mag; // magnitude-weighted chroma (power lets one loud bass note dominate)
    }
    onset[f] = flux;
...
function analysePcm(pcm) {
  ...
  return {
    ...
    // Mastered pop/electronic previews land around RMS 0.2–0.4 in float
    // PCM (measured on real iTunes clips, Sept 2026: Stardust 0.4+,
    // Kalkbrenner ~0.25); mapping /0.45 spreads that across ~0.5–0.95 so
    // club tracks don't all pin at 1.0 and quiet acoustic ones sit low,
    // like Spotify's old energy did.
    energy: Math.round(Math.min(1, Math.max(0, rms / 0.45)) * 100) / 100,
